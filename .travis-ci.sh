...
case "$TARGET" in
  prepare)
    ...
    if [ $WITH_OPAM -eq 1 ] ; then
      ...
      if [ ! -e ~/ocaml/bin/opam -o ! -e ~/.opam/lock -o "$OPAM_RESET" = "1" ] ; then
        ...
        opam install utop ppx_driver ppx_inline_test ppx_expect odoc menhir ocaml-migrate-parsetree js_of_ocaml-ppx --yes
  ...
  build)
    ...
    if [ $WITH_OPAM -eq 1 ] ; then
      ...
      opam install utop ppx_driver ppx_inline_test ppx_expect odoc ocaml-migrate-parsetree js_of_ocaml-ppx --yes
