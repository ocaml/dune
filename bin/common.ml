  ...
  (match action_runner with
   ...
   | Some runner ->
     Dune_engine.Process.Runner.set (fun request ->
       let run_id = Dune_scheduler.Scheduler.current_run_id () in
       Some
         (Dune_scheduler.Scheduler.with_current_build_cancellation_handler
            ~on_cancel:(fun () -> Dune_engine.Action_runner.cancel_build runner ~run_id)
            (fun () -> Dune_engine.Action_runner.exec_process runner ~run_id request))));
