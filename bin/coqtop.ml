...
let run_coqtop ~sctx ~dir ~coqtop ~coq_file_arg ~coq_file_build () =
  let open Memo.O in
  ...
  let coq_module =
    ...
    | None ->
      ...
      in
      User_error.raise ~hints [ Pp.textf "cannot find file: %s" coq_file_arg ]
  in
  ...
  let* args, boot_type =
    ...
    | Some (`Theory theory) ->
      Dune_rules.Coq_rules.coqtop_args_theory ~sctx ~dir ~dir_contents:dc theory
        coq_module
    | Some (`Extraction extr) ->
      Dune_rules.Coq_rules.coqtop_args_extraction ~sctx ~dir ~dir_contents:dc
        extr
  in
...
let coqtop_fiber ~(root : Workspace_root.t) ~prefix_target ~context ~coqtop
    ~coq_file_arg ~extra_args () =
  let open Fiber.O in
  ...
  in
  let* coqtop, args =
    run_coqtop ~sctx ~dir ~coqtop ~coq_file_arg ~coq_file_build
...
let term =
  ...
  let coqtop, argv, env =
    Scheduler.go ~common ~config
      (coqtop_fiber ~root ~prefix_target ~context ~coqtop ~coq_file_arg
         ~extra_args)
  in
