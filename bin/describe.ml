...
module Crawl = struct
  ...
  module Deps : sig
    ...
    let read ~options ~project ~obj_dir ~modules unit =
      let no_deps = ([], []) in
  ...
  let executables sctx ~options ~project ~dir (exes : Dune_file.Executables.t) :
    ...
    let deps_of module_ =
      ...
      Deps.read ~options ~project ~obj_dir ~modules:modules_ module_
    in
    ...
    | Ok requires ->
      ...
      let+ modules_ =
        ...
        | true ->
          ...
          in
          let deps_of module_ =
            Deps.read ~options ~project ~obj_dir:obj_dir_ ~modules:modules_
              (pp_map module_)
          in
