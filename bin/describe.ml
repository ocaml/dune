...
module Crawl = struct
  ...
  module Deps : sig
    ...
    (** [read ~use_pp ~obj_dir ~modules unit] reads the direct dependencies of
        the compilation unit [unit] in the set of modules [modules]. The flag
        [use_pp] indicates whether the compilation unit is subject to a
        preprocessing phase. *)
    let read ~use_pp ~obj_dir ~modules unit =
      let unit =
  ...
  let executables sctx ~project ~dir (exes : Dune_file.Executables.t) :
    ...
    let deps_of =
      ...
      Deps.read ~use_pp ~obj_dir ~modules:modules_
    in
      ...
      let+ modules_ =
        if Lib.is_local lib then
          ...
          let* deps_of =
            ...
            Deps.read ~use_pp ~obj_dir:obj_dir_ ~modules:modules_
          in
