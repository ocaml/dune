...
module Crawl = struct
  ...
  let immediate_deps_of_module ~options ~project ~obj_dir ~modules unit =
    match options.with_deps with
    | false -> Action_builder.return { Ml_kind.Dict.intf = []; impl = [] }
    | true ->
      let deps =
        Dune_rules.Dep_rules.immediate_deps_of ~project unit modules obj_dir
      in
  ...
  let executables sctx ~options ~project ~dir (exes : Dune_file.Executables.t) :
    ...
    let deps_of module_ =
      ...
      immediate_deps_of_module ~options ~project ~obj_dir ~modules:modules_
        module_
    in
    ...
    | Ok requires ->
      ...
      let+ modules_ =
        ...
        | true ->
          ...
          in
          let deps_of module_ =
            immediate_deps_of_module ~options ~project ~obj_dir:obj_dir_
