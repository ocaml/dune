  ...
  module Mod = struct
    ...
    module Origin = struct
      type t =
        ...
        | Dune of Path.t (* generated by a rule; path to the generating dune file *)
        | Lexer of Path.t (* generated by ocamllex; path to the .mll file *)
        | Parser of Path.t (* generated by ocamlyacc/menhir; path to the .mly file *)
        | Wrapper of Path.t
      ...
      let to_dyn : t -> Dyn.t =
        ...
        | Dune p -> variant "dune" [ dyn_path p ]
        | Lexer p -> variant "lexer" [ dyn_path p ]
        | Parser p -> variant "parser" [ dyn_path p ]
...
module Crawl = struct
  ...
  let origin_of_module ~dune_file ~parser_gen_origins (m : Module.t)
    ...
    | Intf_only | Virtual | Impl | Impl_vmodule | Parameter ->
      (match Dune_lang.Module_name.Path.Map.find parser_gen_origins (Module.path m) with
       | Some (kind : Ml_sources.Parser_gen_origin.t) ->
         let ext =
           match kind with
           | Ocamllex -> Filename.Extension.mll
           | Ocamlyacc | Menhir -> Filename.Extension.mly
         in
         let origin path =
       ...
       | None ->
         (match module_source_file m with
          ...
          | Some file ->
            ...
            >>| (function
             | None -> Descr.Mod.Origin.Source
             | Some rule ->
               (match rule.info with
                | Source_file_copy _ -> Source
                | From_dune_file loc when not (Loc.is_none loc) ->
                  Descr.Mod.Origin.Dune (Path.of_string (Loc.start loc).Lexing.pos_fname)
                | From_dune_file _ | Internal ->
                  (* A rule with no user-facing location (internal, or an
                     empty loc); attribute it to the stanza's dune file. *)
                  Dune dune_file))))
  ;;
  ...
  let modules ~obj_dir ~dune_file ~parser_gen_origins ~deps_of modules_
    ...
    |> Modules.fold ~init:(Memo.return []) ~f:(fun m macc ->
      ...
      let* origin = origin_of_module ~dune_file ~parser_gen_origins m in
