  ...
  module Mod = struct
    ...
    module Origin = struct
      type t =
        ...
        | Copy_of_source of Path.t
          (* a copy of a source file, compiled in place of it; path to the file
             it was copied from *)
        | Wrapper of Path.t
      ...
      let to_dyn : t -> Dyn.t =
        ...
        | Copy_of_source p -> variant "copy_of_source" [ dyn_path p ]
...
module Crawl = struct
  ...
  (* Melange compiles a copy of each source file rather than the file itself.
     The copies are made under [Melange.Source.dir] by
     [Melange_rules.setup_melange_sources_copy_rules]. Recover the path a copy
     was made from, so that a module can be described in terms of the file the
     user actually wrote. *)
  let melange_source_copy_original path =
    let open Option.O in
    let* path = Path.as_in_build_dir path in
    let components = Path.Build.explode path |> List.map ~f:Filename.to_string in
    let* () =
      Option.some_if (List.mem components Melange.Source.dir ~equal:String.equal) ()
    in
    List.filter components ~f:(fun component ->
      not (String.equal component Melange.Source.dir))
  ...
  let origin_of_module
    ...
    (* A module compiled in Melange mode is a copy of the file the user wrote,
       so describe that file rather than the internal rule making the copy. *)
    let copied_from =
      match (for_ : Compilation_mode.t), module_source_file m with
      | Ocaml, _ | _, None -> None
      | Melange, Some file -> melange_source_copy_original (Module.File.path file)
    in
    let source_path file = Option.value copied_from ~default:(Module.File.path file) in
    let source =
      match copied_from with
      | None -> Source
      (* Name the file in the source tree, like the other path-carrying
         origins, rather than its build-tree copy. *)
      | Some path -> Copy_of_source (Path.drop_optional_build_context path)
    in
       ...
       | Some kind ->
         ...
         let origin path =
           (* Point at the source-tree generator input (e.g. [foo.mll]), like
              ...
              copy. *)
           Path.set_extension (source_path path) ~ext
  ...
  let modules ~options ~obj_dir ~dune_file ~parser_gen_origins ~deps_of ~for_ modules_
    ...
    |> Modules.fold ~init:(Memo.return []) ~f:(fun m macc ->
      ...
      let* origin = origin_of_module ~options ~dune_file ~parser_gen_origins ~for_ m in
