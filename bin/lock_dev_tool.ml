...
let solve ctx_name ~dev_tool ~local_packages =
  ...
  and* workspace =
    ...
    | `Disabled -> workspace
  in
  let lock_dir = Dune_rules.Lock_dir.dev_tool_lock_dir_path ctx_name dev_tool in
  Memo.of_reproducible_fiber
  @@ solve
       workspace
       ~local_packages
...
let lockdir_status ctx_name dev_tool =
  let open Memo.O in
  let dev_tool_lock_dir = Dune_rules.Lock_dir.dev_tool_lock_dir_path ctx_name dev_tool in
