...
module Make(M : sig val which : which end) = struct
  open Dune
  open Import
  open Cmdliner
  open Fiber.O
  ...
  let set_common c ~targets =
    ...
    Clflags.external_lib_deps_hint :=
      List.concat
        [ [program_name; "external-lib-deps"; "--missing"]
        ; c.orig_args
        ; targets
        ]
...
I couldn't find the opam-installer binary :-("
  ...
  let subst =
    ...
    let man =
      ...
      ; `Pre (sprintf {|  [%S "subst"] {pinned}|} program_name)
      ; `Blocks help_secs
      ]
    in
    let go common name =
  ...
  let default =
    ...
    , Term.info program_name ~doc ~version:"%%VERSION%%"
        ~man:
          [ `S "DESCRIPTION"
