...
module Progress_indicator = struct
  module Per_lockdir = struct
    module State = struct
      ...
      let pp = function
        | Initializing -> Pp.text "Initializing..."
    ...
    let create lockdir_path = { lockdir_path; state = ref State.Initializing }

    let pp { lockdir_path; state } =
      Pp.concat
        [ Pp.textf "Locking %s: " (Path.Source.to_string lockdir_path |> String.quoted)
        ; State.pp !state
        ]
    ;;
  end
  ...
  let pp (t : t) =
    ...
    let line_to_display =
      List.find t ~f:(fun { Per_lockdir.state; _ } ->
        match !state with
        | Done -> false
        | _ -> true)
    in
    match line_to_display with
    | None -> Pp.nop
    | Some line -> Per_lockdir.pp line
  ;;
...
let solve_lock_dir
  ...
  let* repos =
    ...
    let repo_names = Dune_pkg.Pkg_workspace.Repository.Name.Map.keys repo_map in
    progress_state := Progress_indicator.Per_lockdir.State.Updating_repos repo_names;
  ...
  progress_state := Progress_indicator.Per_lockdir.State.Solving;
  ...
  | Ok { lock_dir; files; pinned_packages } ->
    ...
    in
    progress_state := Progress_indicator.Per_lockdir.State.Done;
