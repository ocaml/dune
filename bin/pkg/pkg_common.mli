...
(** Create a [Solver_env.t] by combining variables taken from the current system
    and variables taken from the current context, with priority being given to
    the latter. Some variables are initialized to default values (which can be
    overridden by the arguments to this function):
...
module Version_preference : sig
  type t := Version_preference.t

  val term : Version_preference.t option Term.t
...
val repositories_of_workspace
  :  Workspace.t
  -> Pkg_workspace.Repository.t Pkg_workspace.Repository.Name.Map.t

val repositories_of_lock_dir
  :  Workspace.t
  -> lock_dir_path:Path.Source.t
  -> Pkg_workspace.Repository.Name.t list
...
val get_repos
  :  Pkg_workspace.Repository.t Pkg_workspace.Repository.Name.Map.t
  -> repositories:Pkg_workspace.Repository.Name.t list
  -> update_opam_repositories:bool
  -> Opam_repo.t list Fiber.t

val find_local_packages : Local_package.t Package_name.Map.t Fiber.t
...
(** [pp_packages lock_dir] returns a list of pretty-printed packages occuring in
    [lock_dir]. *)
val pp_packages : Lock_dir.Pkg.t list -> 'a Pp.t
