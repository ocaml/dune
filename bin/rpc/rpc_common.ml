...
let request_exn client request n =
  let* decl = Client.Versioned.prepare_request client request in
  match decl with
...
let notify_exn client notification arg =
  let* res = Client.Versioned.prepare_notification client notification in
  match res with
...
let establish_connection_exn lock_held_by =
  establish_connection ()
  >>| function
  | Ok connection -> connection
  | Error _ -> Dune_util.Global_lock.raise_other_dune_locked lock_held_by
;;

let establish_client_session ~wait ~lock_held_by () =
  if wait
  then establish_connection_with_retry ()
  else establish_connection_exn lock_held_by
;;
