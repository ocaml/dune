...
let make_request ~common ~dir_or_cram_test_paths (setup : Import.Main.build_system) =
  List.map dir_or_cram_test_paths ~f:(fun dir_or_cram_test_path ->
    let dir_or_cram_test_path =
      Path.(relative root) (Common.prefix_target common dir_or_cram_test_path)
    in
    let open Action_builder.O in
    let* alias_kind =
      match Path.as_in_source_tree dir_or_cram_test_path with
      (* If the path is in the source tree, we disambiguate it. *)
      | Some path -> Action_builder.of_memo (disambiguate_test_name path)
      | None -> Action_builder.return (`Runtest dir_or_cram_test_path)
    in
    Alias.request
    @@
