...
let check_path contexts =
  let contexts =
    Build_api.Api.Context_name.Map.of_list_map_exn contexts ~f:(fun c ->
        (c.Context.name, c))
  in
  fun path ->
    let internal_path () =
    ...
    in
    let context_exn ctx =
      match Build_api.Api.Context_name.Map.find contexts ctx with
      | Some context -> context
      | None ->
        User_error.raise
          [ Pp.textf "%s refers to unknown build context: %s"
              (Path.to_string_maybe_quoted path)
              (Build_api.Api.Context_name.to_string ctx)
          ]
          ~hints:
            (User_message.did_you_mean
               (Build_api.Api.Context_name.to_string ctx)
               ~candidates:
                 ( Build_api.Api.Context_name.Map.keys contexts
                 |> List.map ~f:Build_api.Api.Context_name.to_string ))
    in
    match path with
    | External e -> External e
    | In_source_tree s -> In_source_dir s
    | In_build_dir path -> (
      match Build_api.Api.Dpath.analyse_target path with
