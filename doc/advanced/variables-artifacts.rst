...
- ``cmo:<reference>``, ``cmx:<reference>``, and ``cmi:<reference>`` expand to
  the corresponding artifact's path for the specified module. The final
  component of ``<reference>`` is the module's full logical path, independently
  of the dune language version. It is rooted at the selected module tree even
  when used in a nested ``dune`` file. For example, with
  ...
  and selects another module tree, as in ``%{cmi:sub/X}``. To prevent a
  slash-separated source path from silently selecting a different logical
  module, Dune rejects a reference when its complete spelling is the source path
  of another module.
