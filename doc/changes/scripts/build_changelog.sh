...
append_files_in_dir_if_not_empty() {
  ...
  list_of_files=$(find "$dir" -maxdepth 1 -type f -not -name '.*' -name '*.md' | tac)
...
# Add the generated header comment
cat >> "$output" <<EOF
...
# Append the previous changelog, omitting  generated header comment
tail -n +3 CHANGES.md >> "$output"
