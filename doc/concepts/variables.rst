...
- ``ppx:lib1+..+libn`` expands to the ppx executable with ppx libraries
  ...
  this executable.
- ``pkg:<package>:<section>`` expands to the install directory for
  ``<section>`` of ``<package>``. The supported sections are ``lib``,
  ``libexec``, ``bin``, ``sbin``, ``share``, ``etc``, ``doc``, ``man``,
  ``toplevel``, and ``stublibs`` (see :doc:`/reference/dune/install` for
  details on each section). Per the opam convention, ``lib``, ``libexec``,
  ``share``, ``etc``, and ``doc`` append the package name as a subdirectory,
  while ``bin``, ``sbin``, ``man``, ``toplevel``, and ``stublibs`` do not.
  This form also introduces a dependency on the package so that its artifacts
  are built before the action runs.
