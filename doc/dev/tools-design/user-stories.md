...
If I no longer need to use a tool, I can remove it via command line. I should no
longer be able to run the tool.
...
For tools that don't interact with the project dependencies (I3), the tool
...
When I switch contexts, I want my tools to continue working. Compiler
independent tools shouldn't be rebuilt when I change the context.
...
Installing editor tools (ocaml-lsp-server, and whatever) via dune should work
well with my editor (Emacs, Vim, VSCode).

18. Optional Integration

When a tool has an optional feature powered by another package, I want to enable
that feature with the tool, so that I get the capability.
