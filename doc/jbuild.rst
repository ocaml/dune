...
- ``(promote-if <files-to-promote>)`` is the same as ``(promote
  <files-to-promote>)`` except that it does nothing when either of the
  two files of a ``(<a> as <b>)`` form doesn't exist.  This can be
  used with command that only produce a correction when differences
  are found
