...
which corresponds to the environment, in which ``dune`` is run. You can
...
The ``dune-workspace`` file uses the S-expression syntax. This is what a
typical ``dune-workspace`` file looks like:
...
Note that an empty ``dune-workspace`` file is interpreted the same as
one containing exactly:
...
This allows you to use an empty ``dune-workspace`` file to mark the root
of your project.
