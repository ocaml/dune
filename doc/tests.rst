...
.. code:: bash
	@@ -39,7 +39,7 @@ with other targets by passing ``@runtest`` to ``dune build``. For instance:
   $ dune build @install @test/runtest
Running a Single Test
---------------------

If you would only like to run a single test for your project, you may use ``dune
	@@ -51,24 +51,23 @@ exec`` to run the test executable (for the sake of this example,
...
.. code:: scheme
	@@ -86,8 +85,8 @@ so for instance the ``dune`` file might look like this:
    (name foo)
...
.. code:: scheme
	...
	@@ -108,11 +107,11 @@ instance, if we make the test fail by replacing ``120`` by ``0`` we get:
   FAILED 1 / 1 tests
...
field:
.. code:: scheme
	@@ -125,12 +124,12 @@ In the example above, the name `qtest.lib` comes from the `public_name` field
...
.. code:: ocaml
	@@ -140,28 +139,28 @@ expect this code to print. For instance, using ppx_expect_:
       Hello, world!
     |}]
The test procedure consists of executing the OCaml code and replacing
the contents of the ``[%expect]`` extension point by the real
output. That produces a new file that you can compare to the original
...
.. code:: scheme
	@@ -170,8 +169,8 @@ your list of ppx rewriters as follows:
    (inline_tests)
...
.. code:: bash
	@@ -194,17 +193,17 @@ In order to accept the correction, simply run:
   $ dune promote
...
----------------------------------
	@@ -214,21 +213,21 @@ You may also run a group of tests located under a directory with:
...
``deps`` field follows the usual :ref:`deps-field`. For instance:
	@@ -255,11 +254,11 @@ a ``deps`` field the ``inline_tests`` field. The argument of this
...
.. code:: ocaml
	@@ -292,12 +291,12 @@ You can specify such flags by using ``flags`` field. For instance:
The argument of the ``flags`` field follows the :ref:`ordered-set-language`.
...
.. code:: ocaml
	@@ -308,15 +307,15 @@ such libraries using a ``libraries`` field, such as:
     (backend qtest)
     (libraries bar)))
...
``(:include ...)`` forms.
.. code:: ocaml
	@@ -328,15 +327,14 @@ runner itself does not depend on anything itself. This field supports
...
``inline_tests.backend`` field to the library stanza. An inline tests
	@@ -359,41 +357,41 @@ These three parameters can be specified inside the
For instance:
...
In this example, we put tests in comments of the form:
	@@ -413,40 +411,40 @@ The backend for such a framework looks like this:
...
.. code:: scheme
	@@ -457,8 +455,8 @@ command. For instance let's consider this test:
    (alias runtest)
...
generated ``test.output`` file to ``tests.expected`` in the source tree.
	@@ -468,11 +466,11 @@ Alternatively, the :ref:`tests-stanza` also supports this style of tests.
   (tests (names tests))
...
.. code:: bash
	@@ -487,7 +485,7 @@ This provides a nice way of dealing with the usual *write code*,
...
.. code:: scheme
	@@ -514,14 +512,14 @@ To define a standalone test, we create a ``.t`` file. For example, ``foo.t``:
.. code:: bash
...
* Commands - A command starts with 2 spaces followed by a ``$``. It's executed
  ...
  example, there's no output yet.
	@@ -541,14 +539,14 @@ We now see the output of the command:
     testing
...
.. code:: bash
   We create a test artifact called "foo"
     ...
     > foo
     > bar
	@@ -560,16 +558,18 @@ For example, here's an example of how we'd test the ``wc`` utility. ``wc.t``:
...
We can specify dependencies using the ``deps`` field using the usual syntax:
	@@ -610,7 +610,7 @@ We can specify dependencies using the ``deps`` field using the usual syntax:
   (cram
    (deps ../foo.exe))
...
The ``cram`` stanza accepts the following fields:
	@@ -646,15 +646,15 @@ Testing an OCaml Program
------------------------
...
.. code:: scheme
	@@ -664,8 +664,8 @@ To use this binary in the cram test, we should depend on the binary in the test:
Sandboxing
----------
...
artifact that might rely on using the ``deps`` field.
	@@ -674,16 +674,16 @@ See :ref:`dune-action-plugin` for details about the sandboxing mechanism.
...
- The ``PWD`` of the test will be replaced by ``$TESTCASE_ROOT``
	@@ -698,9 +698,9 @@ the standard BUILD_PATH_PREFIX_MAP_ environment variable. For example:
   $ echo $HOME
   $HOME
