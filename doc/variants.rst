...
against the virtual library, and they might only select concrete implementations for these
...
Dune language, and all uses of ``(using library_variants)`` are forbidden
since 2.6.
...
* It isn't possible to load virtual libraries into ``utop``. As a result,
