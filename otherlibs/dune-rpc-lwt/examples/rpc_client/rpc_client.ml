let () =
  ...
  Lwt_main.run
    ...
    Dune_rpc_lwt.V1.Client.connect chan init ~f:(fun client ->
        ...
        let* () =
          let* request =
            Dune_rpc_lwt.V1.Client.Versioned.prepare_request client
        ...
        let* progress_stream =
          Dune_rpc_lwt.V1.Client.poll client Dune_rpc.V1.Sub.progress
          >|= Result.get_ok
        in
        print_endline "Waiting for next progress event...";
        let* progress_event =
          Dune_rpc_lwt.V1.Client.Stream.next progress_stream
        in
        let message =
        ...
        let* () =
          let* shutdown_notification =
            Dune_rpc_lwt.V1.Client.Versioned.prepare_notification client
