...
let dyn_of_mkdir_result = function
  | Already_exists -> Dyn.variant "Already_exists" []
  | Created -> Dyn.variant "Created" []
  | Missing_parent_directory -> Dyn.variant "Missing_parent_directory" []
  | Parent_not_directory -> Dyn.variant "Parent_not_directory" []
  | Already_exists_not_directory -> Dyn.variant "Already_exists_not_directory" []
;;

let mkdir_simple ~perms t_s =
  try
...
let mkdir ?(perms = 0o777) t_s =
  match mkdir_simple ~perms t_s with
  | (Missing_parent_directory | Parent_not_directory | Created) as res -> res
  | Already_exists ->
    if Sys.is_directory t_s then Already_exists else Already_exists_not_directory
  | Already_exists_not_directory ->
    Code_error.raise
      "Impossible happened: mkdir_simple returned Already_exists_not_directory"
      []
;;
...
let rec mkdir_p ?(perms = 0o777) t_s =
  ...
  | Unix.Unix_error (EEXIST, _, _) ->
    if Sys.is_directory t_s then Already_exists else Already_exists_not_directory t_s
  | Unix.Unix_error (ENOENT, _, _) ->
    ...
    (match mkdir_p ~perms parent with
     ...
     | Created | Already_exists -> mkdir_p ~perms t_s)
  | Unix.Unix_error (ENOTDIR, _, _) ->
    (* We are told that a parent is not a directory. We go and find it to give a good
       error message. *)
    let rec find_non_dir_parent t_s =
      let parent = Filename.dirname t_s in
      match mkdir_simple ~perms parent with
      | Created | Missing_parent_directory ->
        (* Since we know that the parent is not a directory, we should never get here. *)
        Code_error.raise
          "Expected parent to not be a directory."
          [ "t_s", Dyn.string t_s; "parent", Dyn.string parent ]
      | Parent_not_directory -> find_non_dir_parent parent
      | Already_exists | Already_exists_not_directory -> parent
    in
