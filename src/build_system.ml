                    ...
                    ; List
                        (Dune_lang.atom "only"
                         :: (Path.Set.to_list inter
                             |> List.map ~f:(fun p ->
                               Dune_lang.Encoder.string (Path.reach ~from:dir p))))
                    ]
              in
              Errors.warn
                (rule_loc ~loc:rule.loc ~dir ~file_tree:t.file_tree)
                "\
The following files are both generated by a rule and present in the source tree:
%s
Because your project was written for dune %s, I am closing my eyes on this
...
longer accept this once you upgrade your project to dune >= 1.10.%s"
                (Path.Set.to_list inter
                 |> List.map ~f:(fun p ->
                   sprintf "- %s\n" (Path.to_string_maybe_quoted p))
                 |> String.concat ~sep:" ")
                (Syntax.Version.to_string dune_version)
                (Dune_lang.to_string mode_def ~syntax:Dune)
                (match rule.loc with
                 | None -> ""
                 | Some _ ->
                   "
...
if you really want to keep these files in your source tree.");
              let mode =
                Dune_lang.Decoder.parse
                  Dune_file.Rule.Mode.decode
                  (Univ_map.singleton (Syntax.key Stanza.syntax) (1, 10))
                  (Dune_lang.add_loc mode_def ~loc:Loc.none)
              in
              { rule with mode }
            end)
      end
