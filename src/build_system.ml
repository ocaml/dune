...
let rule_loc ~loc ~dir =
  ...
  | None ->
    ...
    let file =
      match
        Option.bind (File_tree.find_dir dir)
...
module Alias0 = struct
  ...
  let dep_multi_contexts ~dir ~name ~contexts =
    ignore
      (find_dir_specified_on_command_line ~dir : File_tree.Dir.t);
...
let get_dir_status t ~dir =
  Path.Table.find_or_add t.dirs dir ~f:(fun _ ->
    if Path.is_in_source_tree dir then
      Dir_status.Loaded (File_tree.files_of dir)
...
and load_dir_step2_exn t ~dir ~collector ~lazy_generators =
  ...
  let targets, to_copy, subdirs_to_keep =
    ...
    | ctx_name ->
      ...
      let files, subdirs =
        match File_tree.find_dir sub_dir with
...
let all_targets t =
  String.Map.iter t.contexts ~f:(fun ctx ->
    File_tree.fold ~traverse_ignored_dirs:true ~init:()
      ~f:(fun dir () ->
        load_dir t
