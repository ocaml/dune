...
let add_spec t fn spec ~copy_source =
  ...
  | Some (File_spec.T { rule; _ }) ->
    ...
    | true, Fallback_rule already_present ->
      if Pset.is_empty already_present then
        disabled_fallback_rules := rule :: !disabled_fallback_rules;
      rule.fallback <- Fallback_rule (Pset.add fn already_present);
      Hashtbl.add t.files ~key:fn ~data:spec
    | true, Not_a_fallback_rule ->
      Loc.warn (Internal_rule.loc rule ~dir:(Path.parent fn))
        "%S is both generated by a rule and present in the source tree.\n\
         As a result, the rule is currently ignored, however this will become an error \
         in the future.\n\
         To keep the current behavior and hide this warning, add a field (fallback) \
         to the rule.\n\
         For instance:\n\
         %a@."
        (Path.basename fn)
              ...
              ; List
                  [ List [ Atom"targets"
                         ; Sexp.To_sexp.(list string)
                             (List.map (Pset.elements rule.targets)
                                ~f:Path.basename)
                         ]
                  ; List [ Atom "fallback" ]
                  ; Atom "..."
...
let compile_rule t ~all_targets_by_dir ?(copy_source=false) pre_rule =
  ...
  let rule =
    ...
    ; fallback = if fallback then Fallback_rule Pset.empty else Not_a_fallback_rule
    ; loc
    }
  in
  ...
  (let l = !disabled_fallback_rules in
   ...
   List.iter l ~f:(fun rule ->
     let disabled_for =
       ...
       | Not_a_fallback_rule -> assert false
       | Fallback_rule paths -> paths
     in
