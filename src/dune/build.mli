...
(* In the post-arrow Dune, we will have the selective functor [Build] instead
  of the arrow [Build]. To achieve a gradual transition, we introduce a type
   synonym which makes a unit arrow look like a functor. *)
type 'a s = (unit, 'a) t
...
val source_tree : dir:Path.t -> file_tree:File_tree.t -> (unit, Path.Set.t) t
...
val of_result : ?targets:Path.Build.t list -> ('a, 'b) t Or_exn.t -> ('a, 'b) t
...
val action :
  ?dir:Path.t -> targets:Path.Build.t list -> Action.t -> (_, Action.t) t

val action_dyn :
  ?dir:Path.t -> targets:Path.Build.t list -> unit -> (Action.t, Action.t) t
...
val copy_and_add_line_directive :
  src:Path.t -> dst:Path.Build.t -> (unit, Action.t) t
...
val static_deps :
  (_, _) t -> load_targets_from_dir:(dir:Path.t -> Path.Set.t) -> Static_deps.t

val lib_deps : (_, _) t -> Lib_deps_info.t

val targets : (_, _) t -> Path.Build.Set.t
...
val merge_files_dyn :
  target:Path.Build.t -> (Path.t list * string list, Action.t) t
