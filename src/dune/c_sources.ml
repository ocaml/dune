  ...
  String.Set.fold files ~init ~f:(fun fn acc ->
      ...
      | Not_allowed_until version ->
          ...
          User_error.raise ~loc
            [ Pp.textf
                "Source file %s with extension %s is not allowed before \
                 version %s"
            ...
            let eval (kind : C.Kind.t) (c_sources : C.Source.t String.Map.t)
              ...
              Ordered_set_lang.Unordered_string.eval_loc osl
                ...
                ~parse:(fun ~loc s ->
                  ...
                  if s' <> s then
                    User_error.raise ~loc
                      [ Pp.text
                          "relative part of stub is not necessary and should \
                           be removed. To include sources in subdirectories, \
              ...
              |> String.Map.map ~f:(fun (loc, s) ->
                     ...
                     | None ->
                         ...
                         User_error.raise ~loc
                           [ Pp.textf
                               "%s does not exist as a C source. %s must be \
                                present"
                               s
            ...
            let all =
              String.Map.union c cxx ~f:(fun _ (_loc1, c) (loc2, cxx) ->
                  User_error.raise ~loc:loc2
                    [ Pp.textf
                        "%s and %s have conflicting names. You must rename \
                         one of them."
