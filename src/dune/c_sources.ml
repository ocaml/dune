          ...
          let eval (kind : C.Kind.t) (c_sources : C.Source.t String.Map.t)
                  ...
                  User_error.raise ~loc
                    [ Pp.text
                        "relative part of stub is not necessary and should be \
                         removed. To include sources in subdirectories, use \
                         the include_subdirs stanza"
                    ];
                s')
            ...
            |> String.Map.map ~f:(fun (loc, s) ->
                   ...
                   | None ->
                     ...
                     User_error.raise ~loc
                       [ Pp.textf
                           "%s does not exist as a C source. %s must be present"
                           s
          ...
          let all =
            String.Map.union c cxx ~f:(fun _ (_loc1, c) (loc2, cxx) ->
                User_error.raise ~loc:loc2
                  [ Pp.textf
                      "%s and %s have conflicting names. You must rename one \
                       of them."
