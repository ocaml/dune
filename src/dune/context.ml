...
let create ~(kind : Kind.t) ~path ~env ~env_nodes ~name ~merlin ~targets
    ...
    ~dynamically_linked_foreign_archives ~bisect_enabled ~instrument_with =
  ...
  let create_one ~(name : Context_name.t) ~implicit ~findlib_toolchain ~host
    ...
    let t =
      ...
      ; env
      ; findlib = Findlib.create ~paths:findlib_paths ~version ~lib_config
      ; findlib_toolchain
      ; arch_sixtyfour
      ; install_prefix
      ; stdlib_dir
...
let create_for_opam ~root ~env ~env_nodes ~targets ~profile ~switch ~name
  ...
  create
    ...
    ~bisect_enabled ~instrument_with
