...
module Enabled_for = struct
  ...
  let field =
    field_o "enabled_for" (repeat (map ~f:Language.of_string string))

  let field_ext =
    let+ list_opt = field
    and+ ext_version = Syntax.get_exn syntax
    in
...
let dparse_args =
  let+ loc = loc
  and+ enabled_for = fields Enabled_for.field_ext
  in
  ({ loc; enabled_for }, [])

let dune2_record_syntax =
  let+ loc = loc
  and+ ef = Enabled_for.field
  in
  let enabled_for =
    match ef with
...
let dune2_dec =
  if_list
    ...
    ~else_:(let+ () = keyword "disabled" in None)
...
let field_dune2 =
  field "formatting" dune2_dec ~default:dune2_default
