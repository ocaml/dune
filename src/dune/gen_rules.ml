      ...
      | Some package -> (
          if Package.Name.Set.mem visible_pkgs package.name then
            ...
            (* If the stanza is a hidden public implementation of a visible
               library, turn it into an external variant so that we can
               correctly compute the set of "knonw implementation" when
               generating the dune-package file. *)
            match stanza with
            | Library
