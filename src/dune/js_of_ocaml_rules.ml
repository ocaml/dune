...
type sub_command = Compile | Link | Build_runtime
...
let standalone_runtime_rule cc ~javascript_files ~target ~flags =
  ...
  in
  js_of_ocaml_rule (Compilation_context.super_context cc)
...
let build_cm cctx ~(js_of_ocaml : Dune_file.Js_of_ocaml.t) ~src ~target =
  ...
  if separate_compilation_enabled sctx then
    ...
    in
    [ js_of_ocaml_rule sctx ~sub_command:Compile ~dir ~flags:(Command.Args.dyn flags) ~spec ~target ]
  else
    []
