...
let build_c_file (lib : Library.t) ~sctx ~dir ~expander ~includes
  (loc, src, dst) =
  let ctx = Super_context.context sctx in
  let c_flags =
...
let build_cxx_file (lib : Library.t) ~sctx ~dir ~expander ~includes
  (loc, src, dst) =
  let ctx = Super_context.context sctx in
  let output_param =
    if ctx.ccomp_type = "msvc" then
...
let build_o_files lib ~sctx ~(c_sources : C.Sources.t) ~dir ~expander ~requires
  ~dir_contents =
  let ctx = Super_context.context sctx in
...
let setup_build_archives (lib : Dune_file.Library.t) ~cctx
  ...
  |> Option.iter ~f:(fun m ->
    (* These files needs to be alongside stdlib.cma as the compiler implicitly
      adds this module. *)
    [ (Cm_kind.Cmx, Cm_kind.ext Cmx); (Cmo, Cm_kind.ext Cmo); (Cmx, ext_obj) ]
    |> List.iter ~f:(fun (kind, ext) ->
      let src = Path.build (Obj_dir.Module.obj_file obj_dir m ~kind ~ext) in
