...
module Mode = struct
  ...
  let string : type a. a t -> string -> a =
   fun t s ->
    match t with Single -> Value.String s | Many -> [ Value.String s ]
...
module Var = struct
  ...
  let full_name t =
    match t.payload with None -> t.name | Some v -> t.name ^ ":" ^ v
...
let is_var { template; syntax_version = _ } ~name =
  match template.parts with [ Var n ] -> name = Var.full_name n | _ -> false

let text_only t =
  match t.template.parts with [ Text s ] -> Some s | _ -> None
