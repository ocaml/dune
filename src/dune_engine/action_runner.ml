...
let disconnect t =
  ...
  | Initialized _ ->
    ...
    Dune_trace.emit Action (fun () ->
      Dune_trace.Event.Action.runner_disconnected ~name:(Name.to_string t.name));
    Fiber.return ()
  | Awaiting_initialization ready ->
    t.status <- Closed;
    Dune_trace.emit Action (fun () ->
...
let send_request ~request ~payload t =
  ...
  | Error
      [ { Exn_with_backtrace.exn = Dune_rpc.Response.Error.E { kind = Connection_dead; _ }
        ; _
        }
      ] ->
    let* () = disconnect t in
...
let cancel_build t ~run_id =
  ...
  Dune_trace.emit Action (fun () ->
    Dune_trace.Event.Action.runner_cancel_request_sent ~name:(Name.to_string t.name));
  let+ _result =
...
module Worker = struct
  let exec_action ~name ({ Request.Exec.run_id = _; action } : Request.Exec.t) =
    Dune_trace.emit Action (fun () -> Dune_trace.Event.Action.runner_exec_start ~name);
    let build_deps _ = Code_error.raise "no dynamic actions yet" [] in
    Action_exec.exec action ~build_deps
  ;;

  let cancel_build ~name ({ Request.Cancel_build.run_id = _ } : Request.Cancel_build.t) =
  ...
  let start ~name ~where =
    ...
    let private_menu : Client.proc list =
      ...
      ; Client.Handle_request (Decl.exec, exec_action ~name)
