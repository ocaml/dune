...
module Shared = struct
  ...
  let examine_targets_and_store ~can_go_in_shared_cache ~loc ~rule_digest
      ~execution_parameters ~action ~produced_targets =
    let config = Build_config.get () in
    let module Shared_cache = (val config.shared_cache) in
    Shared_cache.examine_targets_and_store ~can_go_in_shared_cache ~loc
      ~rule_digest ~execution_parameters ~action ~produced_targets
end
