...
module Produced = struct
  ...
  let of_validated =
    let rec collect dir : (unit Path.Local.Map.t Path.Build.Map.t, _) result =
      ...
      | Ok dir_contents ->
        ...
        let+ filenames, dirs =
          Result.List.fold_left
            dir_contents
            ~init:(Path.Local.Map.empty, Path.Build.Map.empty)
            ~f:(fun (acc_filenames, acc_dirs) (filename, kind) ->
              match (kind : File_kind.t) with
              | S_REG ->
                let local = Path.Local.of_string filename in
                Ok (Path.Local.Map.add_exn acc_filenames local (), acc_dirs)
              | S_DIR ->
  ...
  let expand_validated_exn (validated : Validated.t) dir_filename_pairs =
    ...
    let dirs =
      ...
      |> Path.Build.Map.map ~f:(Path.Local.Map.of_list_map_exn ~f:(fun file -> file, ()))
    in
  ...
  let all_files { files; dirs } =
    let disallow_duplicates file _payload1 _payload2 =
      Code_error.raise
        ...
        ; "dirs", Path.Build.Map.to_dyn (Path.Local.Map.to_dyn Dyn.opaque) dirs
        ]
    in
    let files_in_dirs =
      Path.Build.Map.foldi dirs ~init:Path.Build.Map.empty ~f:(fun dir filenames ->
        let paths =
          Path.Build.Map.of_list_exn
            (Path.Local.Map.to_list_map filenames ~f:(fun filename payload ->
               Path.Build.append_local dir filename, payload))
        in
      ...
      (Seq.concat
         (Path.Build.Map.to_seq t.dirs
          |> Seq.map ~f:(fun (dir, filenames) ->
            Path.Local.Map.to_seq filenames
  ...
  let digest { files; dirs } =
    let all_digests =
      ...
      :: Path.Build.Map.to_list_map dirs ~f:(fun _ -> Path.Local.Map.values)
    in
      ...
      try
        ...
        let dirs =
          Path.Build.Map.mapi dirs ~f:(fun dir ->
            Path.Local.Map.mapi ~f:(fun filename ->
              f (Path.Build.append_local dir filename)))
        in
        Some { files; dirs }
      with
      | Short_circuit -> None
  ...
  let collect_digests
    ...
    let dirs =
      Path.Build.Map.mapi dirs ~f:(fun dir ->
        Path.Local.Map.mapi ~f:(fun filename -> f (Path.Build.append_local dir filename)))
    in
