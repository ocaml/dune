  ...
  let fetch_legacy_comments t ~file_contents =
    let rec loop t =
      ...
      | Comment (loc, Legacy) ->
        ...
        let s =
          if file_contents.[start] = '#' && file_contents.[start + 1] = '|'
          then
...
module Parser = struct
  ...
  (* To avoid writing two parsers, one for the Cst and one for the Ast, we
     write only one that work for both.
     ...
     one producing Ast one, so it is the one we want to go fast. As a result,
     we encode comment as special [Ast.t] values and decode them for the
     [Cst.t] parser.
  ...
  let parse ~mode ?(lexer = Lexer.token) lexbuf =
    loop false 0 lexer lexbuf [] |> Mode.make_result mode lexbuf

  let parse_cst ?(lexer = Lexer.token) lexbuf =
    loop true 0 lexer lexbuf [] |> List.map ~f:cst_of_encoded_ast
end
