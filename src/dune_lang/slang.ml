...
(* Simplify a list of blangs for And/Or-like operations.
   ...
   (e.g., And within And), and applies boolean absorption/identity rules:
   ...
   the simplified list (identity elements removed). *)
and simplify_blangs blangs ~flatten_nested ~identity =
  Result.List.concat_map blangs ~f:(fun blang ->
    let simplified = simplify_blang blang in
    Option.value (flatten_nested simplified) ~default:[ simplified ]
    |> Result.List.concat_map ~f:(function
      | Blang.Const b when b <> identity -> Error ()
      | Blang.Const _ -> Ok []
      | b -> Ok [ b ]))
  |> Result.to_option
