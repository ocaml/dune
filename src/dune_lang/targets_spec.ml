...
type 'a t =
  ...
  let decode_target ~allow_directory_targets =
    ...
    in
    let named =
      enter (pair string base_decode) >>| fun (name, (path, kind)) ->
      Named_target.Named (name, (path, kind))
    in
    let anonymous =
      base_decode >>| fun (path, kind) ->
      Named_target.Anonymous (path, kind)
    in
    anonymous <|> named
    ...
    let decode_static ~allow_directory_targets =
      ...
      let targets = List.map targets_named ~f:(function
...
let decode_one_static ~allow_directory_targets =
  ...
  let target = match target_named with
    | Named_target.Anonymous (path, kind) -> (path, kind)
    | Named_target.Named (_, (path, kind)) -> (path, kind)
  in
