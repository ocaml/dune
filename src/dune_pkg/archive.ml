...
module Tar : sig
 ...
 v}
  ...
  val args : 'fmt. 'fmt t -> format:'fmt -> archive:Path.t -> target:Path.t -> string list
end = struct
  type _ kind =
...
module Unzip : sig
  ...
  (* XXX tar should be preferred over unzip *)
  let find =
...
module Extractor : sig
  ...
  val run : archive:Path.t -> target:Path.t -> unit Fiber.t
end = struct
  let no_program_error ~ext ~bin_names =
    User_error.raise
      [ Pp.textf "No program found to extract %s files. Tried:" ext
      ; Pp.enumerate bin_names ~f:Pp.verbatim
      ]
  ;;
    ...
    then
      Io.with_file_in temp_stderr ~f:(fun err_channel ->
        ...
        User_error.raise
          ...
          ; Pp.vbox (Pp.concat_map ~sep:Pp.cut ~f:Pp.paragraph stderr_lines)
          ])
  ;;
  ...
  let run ~archive ~target =
    ...
    | `Zip ->
      ...
      >>= (function
       ...
       | None -> no_program_error ~ext ~bin_names:Unzip.bin_names)
    | (`Tar | `Tar_gz | `Tar_bz2) as format ->
      ...
      >>= (function
       ...
       | Some (Gnu tar | Other tar) ->
         ...
         let args = Tar.args tar ~format ~archive ~target in
         run_cmd ~bin ~args ~archive
       | None -> no_program_error ~ext ~bin_names:Tar.bin_names)
  ;;
end
...
let extract ~archive ~target =
  ...
  let+ () = Extractor.run ~archive ~target:target_in_temp in
  ...
  (* CR-somday Alizter: Add fallback to copy if EXDEV raised here *)
