...
module Command = struct
  type t =
    { bin : Path.t
    ; args : archive:Path.t -> target_in_temp:Path.t -> string list
    }
end
...
let tar_args ~archive ~target_in_temp =
  [ "xf"; Path.to_string archive; "-C"; Path.to_string target_in_temp ]
;;

let zip_args ~archive ~target_in_temp =
  ...
  let command =
    lazy
      (match
       ...
       with
       | Some bin -> { Command.bin; args = tar_args }
  ...
  let command =
    lazy
      (match which "unzip" with
       | Some bin -> { Command.bin; args = zip_args }
       | None ->
         ...
         (match
            (* Test for bsdtar before tar, as if bsdtar is installed then it's
               ...
               tar command anyway, as on BSD, MacOS, and Windows systems, the tar
               command can extract zip archives. *)
...
let extract t ~archive ~target =
  ...
  let args = command.args ~archive ~target_in_temp in
  let+ (), exit_code =
