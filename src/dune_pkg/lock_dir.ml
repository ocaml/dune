...
module Pkg = struct
  ...
  let is_available_on_platform t ~platform =
...
let file_contents_by_path ~portable_lock_dir t =
  ...
  :: (Package_name.Map.to_list t.packages
      |> List.map ~f:(fun (name, pkg) ->
        ...
        ( Package_filename.of_package_name name
...
struct
  ...
  let load_pkg ~version ~lock_dir_path ~solved_for_platforms package_name =
    let open Io.O in
    let pkg_file_path =
    ...
    | Ok () ->
      ...
      let+ packages =
        ...
        >>= Io.parallel_map ~f:(fun package_name ->
          let _loc, solved_for_platforms = solved_for_platforms in
          let+ pkg =
            load_pkg ~version ~lock_dir_path ~solved_for_platforms package_name
          in
          package_name, pkg)
             ...
             let sexp_for_dune_workspace =
               List
                 ...
                 ; List
                     ...
                     ; List [ Atom (Atom.of_string "with_defaults") ]
                     ; List
                         [ Atom (Atom.of_string "extra")
...
let packages_on_platform t ~platform =
  ...
  Package_name.Map.filter t.packages ~f:(fun package ->
    Pkg.is_available_on_platform package ~platform)
;;
