...
module Pkg = struct
  type t =
    ...
    ; post_depends : (Loc.t * Package_name.t) list
    ; info : Pkg_info.t
    ; exported_env : String_with_vars.t Action.Env_update.t list
    }

  let equal
    { build_command; install_command; depends; post_depends; info; exported_env }
    t
    =
    ...
    && List.equal
         (Tuple.T2.equal Loc.equal Package_name.equal)
         post_depends
         t.post_depends
  ...
  let remove_locs
    ...
    ; post_depends = List.map post_depends ~f:(fun (_, pkg) -> Loc.none, pkg)
  ...
  let to_dyn { build_command; install_command; depends; post_depends; info; exported_env }
    =
    Dyn.record
      ...
      ; ( "post_depends"
        , Dyn.list (Dyn.pair Loc.to_dyn_hum Package_name.to_dyn) post_depends )
  ...
  let decode =
    ...
    @@ let+ version = field Fields.version Package_version.decode
       ...
       and+ post_depends =
         field ~default:[] Fields.post_depends (repeat (located Package_name.decode))
       ...
       fun ~lock_dir name ->
         ...
         in
         { build_command; depends; post_depends; install_command; info; exported_env }
  ;;
  ...
  let encode
    ...
    record_fields
      ...
      ; field_l Fields.post_depends Package_name.encode (List.map post_depends ~f:snd)
