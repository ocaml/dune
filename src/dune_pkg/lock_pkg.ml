...
let opam_variable_to_slang
      ...
      ~substitute_missing_packages
      packages
      variable
  =
  let variable_string = OpamVariable.to_string variable in
  let convert_with_package_name ~substitute package_name =
    ...
    | true ->
      (match package_name with
       | Some p ->
         ...
         if substitute && not (Package_name.Map.mem packages_in_solution pkg_name)
         then Ok (Slang.text "")
         else (
           let pform =
             let name = Package_variable_name.of_string variable_string in
             Package_variable.to_pform { Package_variable.name; scope = Package pkg_name }
           in
           Ok (Slang.pform pform))
       | None ->
         let pform =
           let name = Package_variable_name.of_string variable_string in
           Package_variable.to_pform { Package_variable.name; scope = Self }
         in
         Ok (Slang.pform pform))
  in
  match packages with
    ...
    (match is_valid_global_variable_name variable_string with
     ...
     | true ->
       (match Pform.Var.of_opam_global_variable_name variable_string with
        ...
        | None -> convert_with_package_name ~substitute:substitute_missing_packages None))
  | [ package_name ] ->
    convert_with_package_name ~substitute:substitute_missing_packages package_name
  | many ->
    let open Result.O in
    let+ many = Result.List.map many ~f:(convert_with_package_name ~substitute:false) in
...
let opam_fident_to_slang ~loc ~packages_in_solution ~substitute_missing_packages fident =
  let open Result.O in
    ...
    *)
    let+ slang =
      opam_variable_to_slang
        ~loc
        ~packages_in_solution
        ~substitute_missing_packages:false
        packages
        variable
    in
    let condition =
...
let opam_raw_fident_to_slang
  ...
  |> opam_fident_to_slang ~loc ~packages_in_solution ~substitute_missing_packages
;;

let opam_string_to_slang ~package ~loc ~packages_in_solution opam_string =
      ...
      (match Re.Group.get group 0 with
       ...
       | interp
         ...
         opam_raw_fident_to_slang
           ~loc
           ~packages_in_solution
           ~substitute_missing_packages:true
           ident
       | other ->
         Error
           (User_error.make
...
(* Translate an Opam filter into Dune's "Slang" DSL. The main difference between
   ...
   semantics.
*)
let filter_to_blang ~package ~loc ~packages_in_solution filter =
  let filter_to_slang (filter : OpamTypes.filter) =
    match filter with
    | FString s -> opam_string_to_slang ~package ~loc ~packages_in_solution s
    | FIdent fident ->
      opam_fident_to_slang
        ~loc
        ~packages_in_solution
        ~substitute_missing_packages:false
        fident
          ...
          | `Filter filter ->
            let slang =
              let+ slang =
                ...
                | CString s -> opam_string_to_slang ~package ~loc ~packages_in_solution s
                | CIdent ident ->
                  opam_raw_fident_to_slang
                    ~loc
                    ~packages_in_solution
            ...
            Some
              (let+ slang =
                 ...
                 | Some filter ->
                   let+ filter_blang =
                     filter_to_blang ~package ~loc ~packages_in_solution filter
...
let depexts_to_conditional_external_dependencies package ~packages_in_solution depexts =
  let open Result.O in
  List.map depexts ~f:(fun (sys_pkgs, filter) ->
    ...
    in
    let+ condition =
      filter_to_blang ~package ~loc:Loc.none ~packages_in_solution filter
      >>| Slang.simplify_blang
    in
    let enabled_if =
          ...
          | Some filter ->
            let+ blang =
              filter_to_blang
                ...
                ~loc:Loc.none
                ~packages_in_solution:version_by_package_name
                filter
              >>| Slang.simplify_blang
            in
