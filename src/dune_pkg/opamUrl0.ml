...
let is_archive t = Option.is_some (Archive_driver.choose_for_filename t.path)

let local_or_git_or_archive_only url loc =
  match (url : t).backend with
  | `rsync when is_local url -> `Path (Path.of_string url.path)
  | `git -> `Git
  | `http when is_archive url -> `Http_archive
  | `rsync | `http | `darcs | `hg ->
    User_error.raise
      ~loc
