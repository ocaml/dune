...
type t =
  { local_packages : Local_package.t Opam_compatible_package_name.Map.t
  ; lock_dir : Lock_dir.t
  ; version_by_package_name : Package_version.t Opam_compatible_package_name.Map.t
...
let version_by_package_name local_packages (lock_dir : Lock_dir.t) =
  let from_local_packages =
    Opam_compatible_package_name.Map.map
      local_packages
      ~f:(fun (local_package : Local_package.t) ->
        Option.value local_package.version ~default:Lock_dir.Pkg_info.default_version)
  in
  let from_lock_dir =
    Opam_compatible_package_name.Map.map lock_dir.packages ~f:(fun pkg ->
      pkg.info.version)
  in
  let exception Duplicate_package of Opam_compatible_package_name.t in
  try
    Ok
      (Opam_compatible_package_name.Map.union
         from_local_packages
         from_lock_dir
  ...
  | Duplicate_package duplicate_package_name ->
    let local_package =
      Opam_compatible_package_name.Map.find_exn local_packages duplicate_package_name
    in
    Error
      (User_message.make
         ...
         [ Pp.textf
             ...
             (Opam_compatible_package_name.to_string local_package.name)
         ])
;;

let concrete_dependencies_of_local_package_with_test t local_package_name =
  let local_package =
    Opam_compatible_package_name.Map.find_exn t.local_packages local_package_name
  in
  ...
  |> Result.map ~f:Opam_compatible_package_name.Set.of_list
;;

let all_non_local_dependencies_of_local_packages t =
  let open Result.O in
  let+ all_dependencies_of_local_packages =
    Opam_compatible_package_name.Map.keys t.local_packages
    |> Result.List.map ~f:(concrete_dependencies_of_local_package_with_test t)
    |> Result.map ~f:Opam_compatible_package_name.Set.union_all
  in
  Opam_compatible_package_name.Set.diff
    all_dependencies_of_local_packages
    (Opam_compatible_package_name.Set.of_keys t.local_packages)
;;
    ...
    let locked_transitive_closure_of_local_package_dependencies =
      ...
      | Error (`Missing_packages missing_packages) ->
        ...
        Code_error.raise
          ...
          [ "missing package", Opam_compatible_package_name.Set.to_dyn missing_packages ]
    in
    let all_locked_packages =
      Opam_compatible_package_name.Set.of_keys t.lock_dir.packages
    in
    Opam_compatible_package_name.Set.diff
      all_locked_packages
      locked_transitive_closure_of_local_package_dependencies
  in
  if Opam_compatible_package_name.Set.is_empty unneeded_packages_in_lock_dir
  then Ok ()
  else (
    let packages =
      Opam_compatible_package_name.Set.to_list unneeded_packages_in_lock_dir
      |> List.map ~f:(Opam_compatible_package_name.Map.find_exn t.lock_dir.packages)
    in
    Error
      (User_message.make
         ~hints:lockdir_regenerate_hints
         ...
         ; Pp.enumerate packages ~f:(fun (package : Lock_dir.Pkg.t) ->
             Pp.textf
               "%s.%s"
               (Opam_compatible_package_name.to_string package.info.name)
...
let validate_dependency_hash { local_packages; lock_dir; _ } =
  let local_packages =
    Opam_compatible_package_name.Map.values local_packages
    ...
    Error
      (User_error.make
         ...
         ; Pp.textf
             ...
             (Opam_compatible_package_name.to_string any_non_local_dependency.name)
         ])
...
let local_transitive_dependency_closure_without_test =
  let module Top_closure = Top_closure.Make (Opam_compatible_package_name.Set) (Monad.Id)
  in
  fun t start ->
    match
      Top_closure.top_closure
        ~deps:(fun a ->
          ...
          |> List.filter ~f:(Opam_compatible_package_name.Map.mem t.local_packages))
        ~key:Fun.id
        start
    with
    | Ok s -> Opam_compatible_package_name.Set.of_list s
...
let transitive_dependency_closure_without_test t start =
  let local_package_names = Opam_compatible_package_name.Set.of_keys t.local_packages in
  let local_transitive_dependency_closure =
    local_transitive_dependency_closure_without_test
      t
      (Opam_compatible_package_name.Set.inter local_package_names start
  ...
  let non_local_transitive_dependency_closure =
    let non_local_immediate_dependencies_of_local_transitive_dependency_closure =
      ...
      |> Opam_compatible_package_name.Set.to_list
      |> Opam_compatible_package_name.Set.union_map ~f:(fun name ->
        let all_deps =
          concrete_dependencies_of_local_package_without_test t name
          |> Opam_compatible_package_name.Set.of_list
        in
        Opam_compatible_package_name.Set.diff all_deps local_package_names)
    in
    Lock_dir.transitive_dependency_closure
      t.lock_dir
    ...
    | Error (`Missing_packages missing_packages) ->
      Code_error.raise
        ...
        (Opam_compatible_package_name.Set.to_list missing_packages
         |> List.map ~f:(fun p ->
           "missing package", Opam_compatible_package_name.to_dyn p))
  in
  Opam_compatible_package_name.Set.union
    local_transitive_dependency_closure
...
let contains_package t package_name =
  let in_local_packages =
    Opam_compatible_package_name.Map.mem t.local_packages package_name
  in
  let in_lock_dir =
    Opam_compatible_package_name.Map.mem t.lock_dir.packages package_name
  in
...
let all_dependencies t package ~traverse =
  ...
  | `Transitive ->
    ...
    Opam_compatible_package_name.Set.remove closure package
;;
  ...
  | `Transitive ->
    let closure =
      transitive_dependency_closure_without_test
        t
        (Opam_compatible_package_name.Set.singleton package)
    in
    Opam_compatible_package_name.Set.remove closure package
;;
...
let opam_package_dependencies_of_package t package ~which ~traverse =
  ...
  |> Opam_compatible_package_name.Set.to_list_map ~f:(fun package_name ->
    OpamPackage.create
      (Opam_compatible_package_name.to_opam_package_name package_name)
      (Opam_compatible_package_name.Map.find_exn t.version_by_package_name package_name
