...
let decode ~name:_ =
  ...
  enter (fields (
    let+ loc = loc
    and+ coq_lang_version = field "coq" Syntax.Version.decode
    and+ name = field "name" Coq_lib_name.decode
    and+ plugins = field "plugins" ~default:[] (repeat Lib_name.decode_loc)
    and+ theories = field "theories" ~default:[] (repeat Coq_lib_name.decode)
    and+ use_stdlib = field "use_stdlib" bool
    and+ boot = field "boot" bool
    in { coq_lang_version; name; theories; use_stdlib; boot; plugins; loc }
  ))
...
let pp t =
  ...
  let theories = List.map ~f:(fun (_, name) -> Coq_lib_name.encode name) in
  let plugins = List.map ~f:(fun (_, name) -> Lib_name.encode name) in
  let fields = Encoder.record_fields [
    Encoder.field "coq" Syntax.Version.encode t.coq_lang_version;
