...
module Bootstrap : sig
  ...
  let make ~scope ~use_stdlib ~wrapper_name ~coq_lang_version coq_module =
    ...
    let* boot_lib =
      ...
      >>= Coq_lib.DB.resolve_boot ~db ~coq_lang_version
    in
    if use_stdlib
    then (
      match boot_lib with
...
let theory_context ~context ~scope ~coq_lang_version ~name buildable =
  let theory =
    ...
    Coq_lib.DB.resolve coq_lib_db ~db ~coq_lang_version name
  in
  let theories_deps =
...
let extraction_context
  ...
  let theories_deps =
    ...
    Resolve.Memo.List.map
      buildable.theories
      ~f:(Coq_lib.DB.resolve coq_lib_db ~db ~coq_lang_version)
  in
  ...
  let boot =
    let db = Scope.libs scope in
    let* coq_lib_db = coq_lib_db in
    Coq_lib.DB.resolve_boot ~db ~coq_lang_version coq_lib_db
  in
  let theories_deps =
    let open Resolve.Memo.O in
    let+ boot = boot
