...
module Version = struct
  module Num = struct
    ...
    let make version_string =
      ...
      in
      let* revision =
        let revision = Group.get groups 3 in
        revision |> Int.of_string |> function
        | Some revision -> Result.Ok revision
        | None -> Result.Error Pp.(textf "Could not parse int: %S." revision)
      in
      let suffix = Group.get groups 4 in
      Result.Ok { major; minor; revision; suffix }

    let by_name { major; minor; revision; suffix } name : Value.t option =
      match name with
      | "major" -> Some (Int major)
      | "minor" -> Some (Int minor)
      | "revision" -> Some (Int revision)
