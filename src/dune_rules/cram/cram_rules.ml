...
let collect_stanzas =
  let stanzas dir ~f =
    let+ stanzas = Memo.exec stanzas_in_dir dir in
    List.filter stanzas ~f:(fun { stanza; _ } -> f stanza)
  in
      ...
      let* runtest_alias, acc =
        Memo.List.fold_left
          ...
          ~f:(fun (runtest_alias, (acc : Spec.t)) { dir; stanza; prepared } ->
            ...
            | true ->
              let+ { expander; deps = stanza_deps } = Memo.Lazy.force prepared in
              let deps, sandbox, env =
                match stanza_deps with
