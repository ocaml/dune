...
module Ml_kind = struct
  ...
  let dedup_modules_by_obj_name modules =
    let rec loop seen acc = function
      ...
      | module_ :: rest ->
        ...
        else loop (Module_name.Unique.Set.add seen obj_name) (module_ :: acc) rest
    in
    loop Module_name.Unique.Set.empty [] modules
  ;;
    ...
    let per_module =
      ...
      |> Module_name.Unique.Map.mapi ~f:(fun obj_name sourced_module ->
        ...
        | Modules.Sourced_module.Normal module_ when Module.has module_ ~ml_kind:Intf ->
          ...
          dedup_modules_by_obj_name (impl_deps @ intf_deps)
