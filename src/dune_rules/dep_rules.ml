...
let deps_of_module ({ modules; _ } as md) ~ml_kind m =
  ...
  | _ -> (
    ...
    let+ deps = Current.deps_of md ~ml_kind m in
...
let deps_of_vlib_module ({ obj_dir; vimpl; dir; sctx; _ } as md) ~ml_kind m =
  ...
  | Some lib ->
    ...
    in
    Dep_gen.read_deps_of ~obj_dir:vlib_obj_dir ~modules ~ml_kind m
...
let immediate_deps_of ~project unit modules obj_dir ml_kind =
  ...
  | _ ->
    if has_single_file modules then Action_builder.return []
      ...
      let module Current = (val current ~project) in
