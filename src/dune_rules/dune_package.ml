...
let decode ~lang ~dir =
  ...
  and+ sites =
    field ~default:[] "sites"
      (repeat (pair (located Site.decode) Section.decode))
  and+ files =
    field ~default:[] "files"
...
let encode ~dune_version { entries; name; version; dir; sections; sites; files }
  ...
  let sexp =
    record_fields
      ...
      ; field_l "files"
          (pair Section.encode (list Install.Entry.Dst.encode))
          files
      ]
  in
  let list s = Dune_lang.List s in
...
let to_dyn { entries; name; version; dir; sections; sites; files } =
  ...
  record
    ...
    ; ( "files"
      , (list (pair Section.to_dyn (list Install.Entry.Dst.to_dyn))) files )
    ]
  ...
  let load file =
    ...
    Fs_memo.with_lexbuf_from_file file_path ~f:(fun lexbuf ->
        ...
        | exception End_of_file ->
          Error
            (User_message.make
               [ Pp.textf "Unexpected end of file when reading %s."
                   (Path.Outside_build_dir.to_string_maybe_quoted file_path)
               ]))
