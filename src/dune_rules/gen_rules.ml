...
let automatic_subdirs components =
  ...
  | Some comp ->
    if Filename.Map.mem automatic_sub_dirs_map comp
...
let rules_for ~dir ~allowed_subdirs rules =
  { Build_config.Rules.build_dir_only_sub_dirs =
      Build_config.Rules.Build_only_sub_dirs.singleton
        ~dir
...
let gen_melange_emit_rules_or_empty_redirect
  ...
  | None -> Memo.return (Build_config.Redirect_to_parent (rules_for opam_file_rules))
  | Some for_melange ->
    ...
    | None ->
      ...
      | None ->
        gen_melange_emit_rules_or_empty_redirect
          ...
          ~dir
          ~allowed_subdirs:automatic_subdirs
          under_melange_emit_target
      | Some nearest_src_dir ->
        (match String.Map.find automatic_sub_dirs_map (Path.Source.basename src_dir) with
         | Some kind ->
         ...
         | None ->
           let allowed_subdirs, opam_file_rules =
             ...
             | `Project_root ->
               Filename.Set.add automatic_subdirs "opam", Memo.return Rules.empty
    ...
    | Some source_dir ->
      ...
      | Standalone_or_root { directory_targets; contents } ->
        ...
        let build_config subdirs =
          { (rules_for ~dir ~allowed_subdirs:subdirs rules) with directory_targets }
        in
        (match under_melange_emit_target with
         ...
         | Some for_melange ->
           ...
           (match melange_rules with
            ...
            | Some emit ->
              Build_config.Rules
                (Build_config.Rules.combine_exn
                   build_config
