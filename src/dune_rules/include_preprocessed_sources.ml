...
module Lib_or_exes_to_pp = struct
  ...
  let from_exes ~sctx ~scope ~expander ~dir (exes : Dune_file.Executables.t) =
    ...
    let* sources =
      ...
      >>| function
      | ml_sources ->
        Ml_sources.modules ~for_:(Exe { first_exe }) ml_sources |> modules_to_pp
    in
...
let libs_or_exes_to_pp_in_source_tree ~sctx ~scope ~expander ~dirs_to_exclude =
  ...
  >>= fun dir ->
  let+ srcs_to_pp =
    Source_tree_map_reduce.map_reduce dir ~traverse:Sub_dirs.Status.Set.all ~f:(fun dir ->
      ...
      then Memo.return To_pp_list.empty
        ...
        | Some (d : Dune_file.t) ->
          Memo.List.fold_left d.stanzas ~init:To_pp_list.empty ~f:(fun acc stanza ->
    ...
    >>= (function
     ...
     | Some stanzas ->
       let has_include_pp_src_stanza =
         List.exists stanzas.stanzas ~f:(fun stanza ->
           match Stanza.repr stanza with
           | T _ -> true
           | _ -> false)
       in
