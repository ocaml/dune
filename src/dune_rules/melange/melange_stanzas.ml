  ...
  let decode =
    ...
    fields
      (let* loc = loc in
       ...
       and+ alias_loc, alias = located (field_o "alias" Alias.Name.decode)
       and+ module_systems =
       ...
       let () =
         ...
         | None -> ()
         | Some a -> (
           match Alias.Name.equal a implicit_alias with
           | false -> ()
           | true ->
             User_error.raise ~loc:alias_loc
               [ Pp.textf
                   "The alias %S is assigned implicitly by Dune to include all \
                    dependencies generated by all melange.emit stanzas found \
                    in the project. To use an alias for this particular \
                    stanza, use a different name."
                   (Alias.Name.to_string implicit_alias)
               ])
       in
       let preprocess =
         let init =
