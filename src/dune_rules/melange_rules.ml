...
let lib_output_dir ~sctx ~scope ~target_dir lib =
  ...
  let lib_dir =
    ...
    | Private _ -> local ()
    | Public (project, _) -> (
      match Dune_project.equal project (Scope.project scope) with
      | true -> local ()
      | false ->
...
let js_includes ~sctx ~target_dir ~(requires_link : Lib.t list Resolve.t) ~scope
    ~js_ext =
  ...
  let deps_of_lib =
    ...
    fun lib ->
      ...
      let+ virtual_deps =
        ...
        | Some vlib ->
          let* vlib = vlib in
          let dst_dir = lib_output_dir ~sctx ~scope ~target_dir vlib in
          let+ modules =
      ...
      in
      let impl_deps =
        let dst_dir = lib_output_dir ~sctx ~scope ~target_dir lib in
