  ...
  let impl_files, intf_files =
    let make_module dialect name ~original_fn ~fn =
      ...
      let descendant =
        ...
        |> Option.value_exn
      in
      let path_in_build_dir = source ~dir:root_dir ~for_:Melange descendant in
        ...
        let path =
          ...
          | Melange ->
            ...
            let descendant =
              ...
              |> Option.value_exn
            in
            source ~dir:root_dir ~for_ descendant
        in
            ...
            (match
             ...
             | Some descendant ->
               ...
               (match Dialect.DB.find_by_extension dialects ext with
                | Some (dialect, ml_kind) ->
                  let file =
                    let file = source ~dir ~for_ descendant in
