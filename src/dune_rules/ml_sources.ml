...
module Generated_modules = struct
  module Lib_select_deps = struct
    let modules =
      let parse_one_set ~dir (files : (Module_name.Path.t * (Loc.t * Module.File.t)) list)
        =
        match Module_name.Path.Map.of_list files with
        | Ok x -> x
        | Error (module_path, (loc, f1), (_, f2)) ->
          ...
          User_error.raise
            ...
            [ Pp.textf
                "Too many files for module %s in %s:"
                (Module_name.to_string (Nonempty_list.last module_path))
                (Path.Source.to_string_maybe_quoted src_dir)
            ; Pp.textf "- %s" (Path.to_string_maybe_quoted (Module.File.path f1))
            ; Pp.textf "- %s" (Path.to_string_maybe_quoted (Module.File.path f2))
            ]
      in
      fun ~dir ~dialects ~include_subdirs libraries ->
       ...
       dependencies *)
              ...
              (match
               ...
               | Some descendant ->
                 let base_path =
                   ...
                   | Include_subdirs.Include Unqualified | No -> []
                   | Include Qualified ->
                     Path.Local.parent_exn descendant
                     |> Path.Local.explode
                     |> List.map ~f:(fun s ->
                       Module_name.of_string_allow_invalid (loc, s)
                       |> Module_name.Unchecked.validate_exn)
                 in
                 let basename, ext =
                   Path.Build.basename dst |> Filename.split_extension
                 in
                 let module_name =
                 ...
                 (match Dialect.DB.find_by_extension dialects ext with
                  | Some (dialect, ml_kind) ->
                    ...
                    let module_path = Nonempty_list.(base_path @ [ module_name ]) in
                    (match ml_kind with
        ...
        let map =
          ...
          Module_name.Path.Map.merge impls intfs ~f:(fun path impl intf ->
            match impl, intf with
            | None, None -> assert false
            | None, Some (loc, intf) ->
              Some (loc, Module.Source.make path ~impl:None ~intf:(Some intf))
            | Some (loc, impl), None ->
              Some (loc, Module.Source.make path ~impl:(Some impl) ~intf:None)
            | Some (loc, impl), Some (_, intf) ->
              Some (loc, Module.Source.make path ~impl:(Some impl) ~intf:(Some intf)))
        in
        Module_name.Path.Map.foldi
          map
          ~init:Module_trie.empty
          ~f:(fun module_path m acc -> Module_trie.set acc module_path m)
    ;;
  end
  ...
  type t =
    ...
    ; lib_select_deps : (Loc.t * Module.Source.t) Module_trie.t Loc.Map.t
    }
  ...
  let sources { modules; lib_select_deps; _ } ~for_ =
    match Loc.Map.find lib_select_deps for_ with
    | None -> modules
    | Some ms ->
      merge_two
        modules
        (Module_trie.foldi ms ~init:Module_trie.Unchecked.empty ~f:(fun k (_, m) acc ->
           Module_trie.Unchecked.set acc (Nonempty_list.map k ~f:Module_name.unchecked) m))
  ;;
  ...
  let add_generated_modules =
          ...
          | x :: l ->
            (match x with
             ...
             | `Select (loc, y) ->
               loop
                 l
                 { acc with lib_select_deps = Loc.Map.add_exn acc.lib_select_deps loc y }
             | `Ocamllex y -> loop l { acc with ocamllexes = y :: acc.ocamllexes }
             | `Ocamlyacc y -> loop l { acc with ocamlyaccs = y :: acc.ocamlyaccs }
             | `Menhir y -> loop l { acc with menhirs = y :: acc.menhirs })
        in
        fun l ->
          loop
            l
            ...
            ; ocamlyaccs = []
            ; menhirs = []
            ; lib_select_deps = Loc.Map.empty
            }
      in
      fun l ->
        let { modules; ocamllexes; ocamlyaccs; menhirs; lib_select_deps } =
    ...
    in
    fun ~expander ~include_subdirs ~dirs ~dialects modules ->
        ...
        Memo.parallel_map
          ...
          ~f:(fun { Source_file_dir.dir; stanzas; path_to_root; _ } ->
            ...
            Memo.parallel_map stanzas ~f:(fun stanza ->
              Expander.eval_blang expander (enabled_if stanza)
              >>= function
              ...
              | true ->
                (match Stanza.repr stanza with
                 ...
                 | Library.T { buildable; _ } ->
                   let dep_info =
                     Lib_select_deps.modules
                       ~dir
                       ~dialects
                       ~include_subdirs
                       buildable.libraries
                   in
                   Memo.return (`Select (buildable.loc, dep_info))
                 | Melange_stanzas.Emit.T mel ->
                   let dep_info =
                     Lib_select_deps.modules ~dir ~dialects ~include_subdirs mel.libraries
                   in
                   Memo.return (`Select (mel.loc, dep_info))
            ...
            (match Stanza.repr stanza with
             | Library.T lib ->
               ...
               let+ sources, modules =
                 ...
                 let modules =
                   Generated_modules.sources modules ~for_:lib.buildable.loc
                 in
                 make_lib_modules
                   ~expander
                   ~dir
             ...
             | Executables.T exes ->
               let modules = Generated_modules.sources modules ~for_:exes.buildable.loc in
               make_executables ~dir ~expander ~modules ~project exes
             | Tests.T tests ->
               let modules =
                 Generated_modules.sources modules ~for_:tests.exes.buildable.loc
               in
               make_tests ~dir ~expander ~modules ~project tests
             | Melange_stanzas.Emit.T mel ->
               let obj_dir = Obj_dir.make_melange_emit ~dir ~name:mel.target in
               let+ sources, modules =
                 let modules = Generated_modules.sources modules ~for_:mel.loc in
