...
let build_module ~dep_graphs ?(precompiled_cmi = false) cctx m =
  (* cwong: This is not the cleanest place to put this, but I think it is the
     simplest. I'm not entirely sure how this interacts with the [best]
     compilation mode -- it may lead to cmts being generated by native rules
     even if bytecode is specified. *)
  let { Mode.Dict.byte; _ } = CC.modes cctx in
  ...
  in
  (* We ensure that cmts are generated by exactly one of the rules by using the
     negation of the [output_cmt] parameter above. *)
  ...
  | Some _, false ->
    build_cm cctx m ~dep_graphs ~precompiled_cmi ~cm_kind:Cmx
      ~phase:(Some Fdo.All) ~output_cmt:byte
  | Some _, true ->
    build_cm cctx m ~dep_graphs ~precompiled_cmi ~cm_kind:Cmx
      ~phase:(Some Fdo.Compile) ~output_cmt:byte;
    Fdo.opt_rule cctx m;
    build_cm cctx m ~dep_graphs ~precompiled_cmi ~cm_kind:Cmx
      ~phase:(Some Fdo.Emit) ~output_cmt:byte );
