...
let alias_source modules ~renames =
  ...
  let+ renames, shadows =
    ...
    in
    let shadows =
      List.sort_uniq shadows ~compare:Module_name.compare
      |> List.filter_map ~f:(fun m ->
             if Module_name.Map.mem aliased_modules m then
               None
             else
               Some
                 (sprintf
                    (* TODO perhaps we should include which library is
                       shadowing? *)
                    "module %s = struct let this_module_is_shadowed = () end"
                    (Module_name.to_string m)))
    in
    (List.rev aliases, shadows)
  in
  [ renames; shadows; internal ] |> List.concat |> String.concat ~sep:"\n\n"

let build_alias_module ~alias_module ~cctx =
  let sctx = Compilation_context.super_context cctx in
  let file = Option.value_exn (Module.file alias_module ~ml_kind:Impl) in
  let modules = Compilation_context.modules cctx in
  (* TODO should be lazy, but we can't compose that with Build.of_result *)
  let alias_file =
    let renames = Compilation_context.renames cctx in
    alias_source modules ~renames
  in
  let dir = Compilation_context.dir cctx in
  (* TODO obtain loc from buildable used to construct cctx *)
  Super_context.add_rule ~loc:Loc.none sctx ~dir
    (let target = Path.as_in_build_dir_exn file in
     Build.With_targets.of_result_map alias_file ~targets:[ target ]
