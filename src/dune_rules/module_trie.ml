...
let to_list_map t ~f = foldi t ~init:[] ~f:(fun key x acc -> f key x :: acc)

let is_empty t =
  match fold t ~init:() ~f:(fun _ _ -> raise_notrace Exit) with
  | () -> false
  | exception Exit -> true

let exists t ~f =
  match
    fold t ~init:() ~f:(fun v acc -> if f v then raise_notrace Exit else acc)
  with
  | () -> false
  | exception Exit -> true
...
let values = fold ~init:[] ~f:(fun v acc -> v :: acc)

let merge x y ~f =
  let base _path _ = assert false in
...
let rec encode t : Dune_lang.t list =
  Module_name.Map.to_list_map t ~f:(fun _ t ->
      Dune_lang.List
        (match t with
        | Leaf m -> Dune_lang.atom "module" :: Module.encode m
        | Map m -> Dune_lang.atom "map" :: encode m))
...
let decode ~src_dir =
  ...
  let rec t =
    lazy
      (let+ modules = either (repeat (sum node)) old in
       match modules with
       | Right map -> map
       | Left modules -> Module_name.Map.of_list_exn modules)
  and node =
    [ ( "module"
      , let+ m = Module.decode ~src_dir in
        (Module.name m, Leaf m) )
    ; ( "map"
...
module Memo_traversals = struct
  let parallel_map t ~f =
    let open Memo.O in
    let rec loop t path =
      Module_name.Map_traversals.parallel_map t ~f:(fun name v ->
          match v with
          | Leaf a ->
            let+ a = f (List.rev (name :: path)) a in
            Leaf a
          | Map m ->
            let+ m = loop m (name :: path) in
            Map m)
    in
    loop t []
end
