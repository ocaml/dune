...
let compile_mld_to_odoc sctx (m : Mld.t) ~includes ~doc_dir ~pkg =
  ...
  let mld_file = Mld.mld_file m in
  let* odoc = odoc sctx in
  let* odoc_base_flags = odoc_base_flags sctx mld_file in
  let+ () =
    add_rule sctx
...
let link_odoc_file sctx odoc_file' ~pkg ~includes:file_deps =
  ...
  in
  let odoc_file = Path.build odoc_file'.odoc_file in
  let* odoc = odoc sctx in
  let open Action_builder.With_targets.O in
  add_rule sctx
    (Action_builder.with_no_targets file_deps
    >>> Action_builder.with_no_targets (Action_builder.path odoc_file)
...
let setup_html sctx odoc_artifact =
  let ctx = Super_context.context sctx in
  let to_remove, dune_keep =
    match odoc_artifact.source with
    | Mld -> (odoc_artifact.html_dir, [])
    | Module ->
      ...
      let dummy =
        Action_builder.create_file (odoc_artifact.html_dir ++ ".dune-keep")
      in
      (odoc_artifact.html_dir, [ dummy ])
  in
  let odocl_file = Path.build odoc_artifact.odocl_file in
  let* odoc = odoc sctx in
  add_rule sctx
    (let open Action_builder.With_targets.O in
    Action_builder.with_no_targets (Action_builder.path odocl_file)
          ...
          (Action_builder.with_no_targets
             (Action_builder.return
                ...
                (Action.Progn
                   ...
                   ; Action.Mkdir (Path.build odoc_artifact.html_dir)
                   ]))
           ::
           Command.run
             ...
             ; Dep odocl_file
             ; Hidden_targets [ odoc_artifact.html_file ]
             ]
           :: dune_keep))
...
let create_odoc_artifact ctx ~target odoc_file odocl_file =
...
let check_mlds_no_dupes ~pkg ~mlds =
  ...
  | Error (_, p1, p2) ->
    User_error.raise
      [ Pp.textf "Package %s has two mlds with the same basename %s, %s"
...
let setup_library_odoc_rules cctx (library : Library.t) ~dep_graphs =
  ...
  let* () =
    Memo.Build.parallel_iter odoc_artifacts
...
let setup_lib_html_rules_def =
  ...
  let f (sctx, lib) =
    ...
    let* () = Memo.Build.parallel_iter odoc_artifacts ~f:(setup_html sctx) in
    let html_files =
...
let setup_pkg_html_rules_def =
  ...
  Memo.With_implicit_output.create "setup-package-html-rules"
    ...
    (fun (sctx, pkg, (libs : Lib.Local.t list)) ->
      ...
      and* pkg_html_artifacts = odoc_artifacts sctx (Pkg pkg) in
      let* () = Memo.Build.parallel_iter pkg_html_artifacts ~f:(setup_html sctx)
      and* lib_odoc_artifacts =
        Memo.Build.parallel_map libs ~f:(fun lib ->
            odoc_artifacts sctx (Lib lib))
      in
      let html_artifacts =
        List.concat (pkg_html_artifacts :: lib_odoc_artifacts)
      in
      let html_files =
...
let gen_rules sctx ~dir:_ rest =
  ...
  | "_odoc" :: "pkg" :: pkg :: _ -> (
    ...
    | Some _ -> setup_package_odoc_rules sctx ~pkg ~libs)
  | "_odoc" :: "lib" :: lib :: _ -> (
