...
module Pkg = struct
  ...
  let base_env t =
    Env.Map.of_list_exn
      ...
      ; (* The relocatable compiler's process.sh (from 5.5.0 onwards) script
           references $OPAMSWITCH. Since dune is not opam, we set it to a value
           that allows the script to progress. It doesn't matter if this switch
           actually exists, since other mechanisms in the script will cause it
           to abort early and avoid cloning the compiler switch. *)
