...
module DB = struct
  ...
  module Pkg_table = struct
    ...
    (* Result of resolving a single declared dependency by name. Anything not
       found in the lock dir, dune itself, or the system-provided set is
       presumed to be a workspace package; if it's not actually present in
       the workspace either, the build will surface the error at consumption
       time. *)
