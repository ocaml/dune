...
module Id = struct
  module T = struct
    ...
    let compare t { path; name; _ } =
      ...
      Rocq_lib_name.compare t.name name
    ;;
    ...
    let to_dyn { path; loc; name } =
      Dyn.(
        record
          ...
          ; "loc", Loc.to_dyn loc
          ; "name", Rocq_lib_name.to_dyn name
          ])
    ;;
  end
  ...
  let pp { path; loc; name } =
    ...
    Pp.concat
      ~sep:Pp.space
      [ Pp.textf "theory %s in" (Rocq_lib_name.to_string name); loc_or_path ]
  ;;
...
module Error = struct
  ...
  let duplicate_boot_lib theories =
    let open Rocq_stanza.Theory in
    let name (t : Rocq_stanza.Theory.t) =
      let name = Rocq_lib_name.to_string (snd t.name) in
  ...
  let duplicate_boot_path paths =
    let describe_path cp =
      ...
      let path = Rocq_path.path cp in
      Pp.textf "%s at %s" (Rocq_lib_name.to_string name) (Path.to_string path)
    in
    User_error.raise
      [ Pp.textf "Found more than one installed Rocq Corelib"
...
module DB = struct
  ...
  module Resolve_result = struct
    (** This is the first result of resolving a rocq library, later one we will
        refine this data. *)
    type 'a t =
      | Redirect of 'a
  ...
  let rec boot_library_id rocq_db =
    match rocq_db.boot_id with
  ...
  module rec R : sig
    ...
    let resolve_boot ~coq_lang_version ~rocq_db (boot_id : Id.t option) =
      match boot_id with
      | Some boot_id ->
        let open Resolve.Memo.O in
        let+ lib = resolve ~coq_lang_version rocq_db (boot_id.loc, boot_id.name) in
    ...
    let resolve_theory
      ...
      =
      let open Resolve.Memo.O in
      let* theory = resolve ~coq_lang_version rocq_db (loc, theory_name) in
    ...
    let resolve_theories ~coq_lang_version ~allow_private_deps ~rocq_db ~boot_id theories =
      let f = resolve_theory ~coq_lang_version ~allow_private_deps ~rocq_db ~boot_id in
    ...
    let create_from_stanza_impl (rocq_db, db, dir, (s : Rocq_stanza.Theory.t)) =
      ...
      and+ boot = resolve_boot ~coq_lang_version ~rocq_db boot_id in
    ...
    module Resolve_final_result = struct
      (** Next we find corresponding Rocq libraries for the various cases *)
      type t =
    ...
    let find rocq_db ~coq_lang_version name : Resolve_final_result.t =
      ...
      | Legacy_theory cp when Rocq_path.corelib cp -> Found_path cp
    ...
    let resolve rocq_db ~coq_lang_version (loc, name) =
      match find rocq_db ~coq_lang_version name with
    ...
    let resolve_boot rocq_db ~coq_lang_version =
      let boot_id = boot_library_id rocq_db in
      resolve_boot ~coq_lang_version ~rocq_db boot_id
    ;;
  end
  ...
  let select_boot_id entries =
    match
      List.find_all entries ~f:(fun ((theory : Rocq_stanza.Theory.t), _entry) ->
        theory.boot)
    with
    | [] -> None
    | [ ((theory, entry) : Rocq_stanza.Theory.t * Entry.t) ] ->
      (match entry with
  ...
  let create_from_rocqlib_stanzas
        ~(parent : t option)
        ~find_db
  ...
  let select_boot_path (cp : Rocq_path.t list) =
    match List.find_all cp ~f:Rocq_path.corelib with
    | [] -> None
    | [ stdlib ] ->
      let name = Loc.none, Rocq_path.name stdlib in
      let path = Rocq_path.path stdlib in
      Some (Id.create ~path ~name)
    | paths -> Error.duplicate_boot_path paths
  ;;

  let create_from_rocqpaths cps =
    (* No parent for rocqpaths for now *)
    let parent = None in
    let boot_id = select_boot_path cps in
    (* Map is eager, this may not be very convenient in large trees,
       but should be fine for now *)
    let map =
      List.rev_map cps ~f:(fun cp -> Rocq_path.name cp, cp)
      (* Since we reversed the order, the second rocqpath should take
