...
let rec physical ~project ~dir ~dirs_visited ~dirs ~sub_dirs ~dune_file ~parent_status =
  let status_map =
...
and contents
  ...
  in
  { Dir0.project; status = dir_status; path; files; sub_dirs; dune_file }

and find_dir_raw ~virtual_ ~dune_file ~status ~dirs_visited ~project ~path ~basename
  : Dir0.t Memo.t
  =
  let status =
  ...
  in
  contents readdir ~path ~dune_file ~dirs_visited ~project ~dir_status:status
;;
...
let root =
  ...
  let* dirs_visited =
    ...
    | Error unix_error -> error_unable_to_load ~path unix_error
  in
  contents readdir ~path ~dune_file:None ~dirs_visited ~project ~dir_status
;;
