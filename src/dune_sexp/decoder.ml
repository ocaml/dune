...
type (_, _) parser =
  ...
  | Arr : ('kind context -> 'kind -> 'a * 'kind) -> ('a, 'kind) parser
...
let fix f =
  let rec p = lazy (f r)
  and r = Arr (fun ctx state -> eval (Lazy.force p) ctx state) in
  r
;;
...
let map_validate t ~f =
  Arr
    (fun ctx state1 ->
      let x, state2 = eval t ctx state1 in
      match f x with
...
let leftover_fields : (Ast.t list, Fields.t) parser =
  Arr
    (fun ctx (state : Fields.t) ->
      eval (leftover_fields_generic raw (Name.Map.keys state.unparsed)) ctx state)
;;
...
let fields_mutually_exclusive ?on_dup ?default fields =
  Arr
    (fun ((Fields (loc, _, _) : _ context) as ctx) state ->
      let res, state =
        eval
