...
type (_, _) parser =
  ...
  | Enter : ('a, values) parser -> ('a, values) parser
...
and eval : type a k. (a, k) parser -> k context -> k -> a * k =
  ...
  | Enter t -> eval_enter t ctx state
  | Peek ->
    (match state with
     | [] -> None, state
  ...
  | Fields_parser t ->
    ...
    let x =
      ...
      result ctx (eval t ctx (Fields.of_values state))
    in
    x, []
  | Leftover_fields_generic { names = more_fields; t } ->
    leftover_fields more_fields t ctx state
  | Traverse (l, f) -> eval_traverse l f ctx state
  | Map_validate (t, f) ->
    let x, state2 = eval t ctx state in
    (match f x with
...
and leftover_fields
  ...
  =
  fun more_fields t (Fields (loc, cstr, uc)) state ->
  let x =
    let ctx = Values (loc, cstr, uc) in
    result ctx (eval_repeat t [] ctx (Fields.unparsed_ast state))
  in
  x, Fields.leftover_fields state more_fields

and eval_traverse
  : type a b c. a list -> (a -> (b, c) parser) -> c context -> c -> b list * c
  =
  fun l f ctx state ->
  List.fold_map ~init:state l ~f:(fun state x -> Tuple.T2.swap (eval (f x) ctx state))
  |> Tuple.T2.swap

and fields_mutually_exclusive
  : type a.
    on_dup:(Univ_map.t -> string -> Ast.t list -> unit) option
    -> default:a option
    -> fields:(string * a t) list
    ...
    -> a * fields
  =
  fun ~on_dup ~default ~fields ((Fields (loc, _, _) : _ context) as ctx) state ->
  let res, state =
    eval_traverse
      fields
      (fun (name, parser) -> field_o name ?on_dup parser >>| fun res -> name, res)
      ctx
      state
  in
  match
  ...
  | [] ->
    (match default with
     | None -> List.map fields ~f:fst |> fields_missing_need_exactly_one loc
     | Some default -> default, state)
  | [ (_name, res) ] -> res, state
  | _ :: _ :: _ as results ->
    List.map ~f:fst results |> fields_mutual_exclusion_violation loc
...
and eval_repeat : type a. a t -> a list -> values context -> values -> a list * values =
  ...
  | [] -> List.rev acc, []
  | _ ->
    let x, state = eval t ctx state in
...
and sum
  : type a.
    force_parens:bool
    -> (string * (a, values) parser) list
    -> values context
    -> values
    -> a * values
  =
  fun ~force_parens cstrs (Values (_, _, uc) as ctx) (state : Ast.t list) ->
  match state with
  | [] -> end_of_list ctx
  | sexp :: sexps ->
    let res =
...
and enum
  : type a. (string * (a, values) parser) list -> values context -> values -> a * values
  =
  fun cstrs (Values (_, _, uc) as ctx) (state : Ast.t list) ->
  match state with
  | [] -> end_of_list ctx
  | sexp :: sexps ->
    let res =
...
and keyword : string -> values context -> values -> unit * values =
  fun kwd ctx state ->
  match state with
  | [] -> end_of_list ctx
  | sexp :: sexps ->
    ...
    in
    (* Silly that we need to create this tuple *)
    (), sexps
;;
