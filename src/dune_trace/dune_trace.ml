...
module Out = struct
  ...
  let create path = Raw_out.create (enabled_categories ()) path
  let of_fd fd = Raw_out.of_fd (enabled_categories ()) fd
end
...
let at_exit =
  At_exit.at_exit Global_lock.at_exit (fun () ->
    ...
    | Some { out; ownership = Owned } ->
      ...
      Out.close out;
      (match
         Env.(get initial Dune_action_trace.Private.trace_dir_env_var), Out.path out
       with
       | None, _ | _, None -> ()
       | Some dir, Some path ->
