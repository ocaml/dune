...
module Tui = struct
  ...
  module Message_viewer = struct
    ...
    (* We calculate the max message length from all the messages. This is used to keep
       the horizontal lines with a consistent length. *)
    ...
    (* We approximate the first line of the message. Unfortunately due to the way Notty
       images work, it is not easy to get the actual width of the first line. Therefore
       we just chop a third off as an approximation. *)
    ...
    let horizontal_line_with_count total index =
      ...
      let toggle_indicator =
        I.hcat
          [ I.uchar divider_attr Unicode.ogham_reversed_feather_mark 1 1
          ; (if index_is_hidden
             then I.string helper_attr "+"
             else I.string helper_attr "-")
          ; I.uchar divider_attr Unicode.ogham_feather_mark 1 1
          ]
      in
      let synopsis =
    ...
    (* We take all the messages in the console and display them as
       [line_separated_message]s. We also handle arrow keys/vim bindings together with
       mouse scrolling for the scrolling effect. We handle ['m'] as an expand /
       collapse all messages function. *)
    let ui =
