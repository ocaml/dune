...
module Tui = struct
  ...
  let horizontal_line_with_count ~w total index =
    ...
    in
    I.(hsnap ~align:`Left w status </> uchar A.(fg red) (Uchar.of_int 0x2015) w 1)
  ;;
  ...
  let image ~status_line ~messages =
    ...
    in
    let debug_image = if ui_state.debug then [ I.empty; debug_image () ] else [] in
  ...
  let horizontal_scroll_bar width =
    ...
    then (
      let nib =
        let l = int_of_float (ui_state.hscroll_pos *. float_of_int (twidth - 6)) + 1 in
      ...
      I.zcat
        ...
        ; I.char A.(fg yellow ++ bg black) ' ' 1 1 |> I.hsnap ~align:`Right (twidth - 1)
        ])
    else I.empty
  ;;
  ...
  let vertical_scroll_bar height =
    ...
    then (
      let nib =
        let t = int_of_float (ui_state.vscroll_pos *. float_of_int (theight - 5)) + 1 in
      ...
      I.zcat
        ...
        ; I.char A.(fg yellow ++ bg black) ' ' 1 1 |> I.vsnap ~align:`Bottom (theight - 1)
        ])
    else I.empty
  ;;
  ...
  let top_frame image =
    ...
    in
    I.zcat [ help_screen (); vertical_scroll_bar h; horizontal_scroll_bar w; image ]
  ;;
