...
module Gen(P : Params) = struct
  ...
  let copy_files_rules (def: Copy_files.t) ~src_dir ~dir ~package_context:_ =
    ...
    ; flags    = []
    ; preprocess = Jbuild_types.Preprocess.No_preprocessing
    ; libname = None
  ...
  let rules { SC.Dir_with_jbuild. src_dir; ctx_dir; stanzas; pkgs = package_context } =
    ...
    let merlin_info =
      List.filter_map stanzas ~f:(fun stanza ->
        ...
        | Rule         rule  -> user_rule   rule  ~dir ~package_context; None
        | Alias        alias -> alias_rules alias ~dir ~package_context; None
        | Copy_files def ->
          Some (copy_files_rules def ~src_dir ~dir ~package_context)
        | Library _ | Executables _ | Provides _ | Install _ -> None) in
      let files = lazy (
