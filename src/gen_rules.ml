...
|}
  ...
  let library_rules (lib : Library.t) ~dir ~files
    ...
    Module_compilation.build_modules sctx
      ~js_of_ocaml ~dynlink ~flags ~scope:scope.data ~dir ~obj_dir ~dep_graph ~modules ~requires ~alias_module;
    Option.iter alias_module ~f:(fun m ->
    ...
    List.iter Mode.all ~f:(fun mode ->
      build_lib lib ~scope:scope.data ~flags ~dir ~obj_dir ~mode ~modules ~dep_graph);
    (* Build *.cma.js *)
    SC.add_rules sctx (
  ...
  let executables_rules (exes : Executables.t) ~dir ~all_modules
    ...
    Module_compilation.build_modules sctx
      ...
      ~dynlink:true ~flags ~scope:scope.data ~dir ~obj_dir ~dep_graph ~modules
      ~requires ~alias_module:None;

    List.iter exes.names ~f:(fun name ->
      List.iter Mode.all ~f:(fun mode ->
        build_exe ~js_of_ocaml:exes.buildable.js_of_ocaml ~flags ~scope:scope.data
          ~dir ~obj_dir ~requires ~name ~mode ~modules ~dep_graph ~link_flags:exes.link_flags
