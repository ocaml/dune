...
module Dst : sig
  ...
  let install_path t section p =
    Path.relative (Paths.get t section) (to_string p)
end
...
let adjust_dst_gen =
  let error (source_pform : Dune_lang.Template.Pform.t) =
    User_error.raise ~loc:source_pform.loc
      [ Pp.textf
          "Because this file is installed in the 'bin' section, you cannot use \
           the %s %s in its basename."
...
let make_with_dst section dst ~kind ~src =
  { optional = false; src; dst; section; kind }
