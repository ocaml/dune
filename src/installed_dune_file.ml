...
let parse_sub_systems ~parsing_context sexps =
  List.filter_map sexps ~f:(fun sexp ->
    let name, ver, data =
      Galach.Of_sexp.(parse (triple string (located Syntax.Version.dparse) raw)
    ...
    Option.map (Sub_system_name.get name) ~f:(fun name ->
      (name, (Galach.Ast.loc sexp, ver, data))))
  ...
  |> Sub_system_name.Map.mapi ~f:(fun name (_, version, data) ->
    ...
    in
    M.T (Galach.Of_sexp.parse M.parse parsing_context data))
...
let load fname =
  Io.with_lexbuf_from_file fname ~f:(fun lexbuf ->
    (* Installed dune files are versioned but they don't use the
       ...
       the file. *)
    let state = ref 0 in
    let lexer = ref Galach.Lexer.token in
    let lexer lb =
      let token : Galach.Lexer.Token.t = !lexer lb in
      (match !state, token with
       | 0, Lparen -> state := 1
       | 1, Atom (A "dune") -> state := 2
       | 2, Atom (A "1") -> state := 3; lexer := Galach.Lexer.jbuild_token
       | 2, Atom (A "2") -> state := 3; lexer := Galach.Lexer.token
       | 2, Atom (A version) ->
      ...
      token
    in
    Galach.Of_sexp.parse of_sexp Univ_map.empty
      (Galach.Parser.parse ~lexer ~mode:Single lexbuf))
...
let gen ~(dune_version : Syntax.Version.t) confs =
  let sexps =
    ...
    |> List.map ~f:(fun (name, (ver, conf)) ->
      let (module M) = Dune_file.Sub_system_info.get name in
      Galach.List [ Galach.atom (Sub_system_name.to_string name)
                ; Syntax.Version.dgen ver
                ; conf
                ])
  in
  Galach.List
    [ Galach.unsafe_atom_of_string "dune"
    ; Galach.unsafe_atom_of_string
