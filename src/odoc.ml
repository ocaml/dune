...
module Gen (S : sig val sctx : SC.t end) = struct
  ...
  let to_html sctx (odoc_file : odoc) ~deps =
...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  let create_odoc ~odoc_input m =
    let html_alias = Dep.html_alias m in
    let html_base = Paths.html m in
    match m with
    | Lib _ ->
      let html_dir =
        html_base ++ (
        ...
        let odocs =
          ...
          List.concat (
            (List.map ~f:(fun odoc_input ->
               create_odoc (Pkg pkg) ~odoc_input
             ) (odocs (Paths.odocs (Pkg pkg))))
            :: (
              List.map libs ~f:(fun lib ->
                odocs (Paths.odocs (Lib lib))
                |> List.map ~f:(fun odoc_input ->
                  create_odoc (Lib lib) ~odoc_input
                )
              )
            )) in
        let html_files =
          ...
          List.map odocs ~f:(fun odoc_file ->
            to_html sctx odoc_file ~deps) in
        List.iter (
