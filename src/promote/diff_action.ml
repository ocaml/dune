...
let diff_eq_files { Diff.optional; mode; file1; file2; _ } =
...
let exec ~rule_loc ({ Diff.optional; file1; file2; mode; force_source } as diff) =
  ...
  then (
    ...
    in
    if force_source && not (is_copied_from_source_tree file1)
    then (
      let file1 =
      ...
      User_error.raise
        [ Pp.textf
            "File %S cannot be generated by another rule, it must be a source file."
            file1
        ])
    else (
...
module Spec = struct
  ...
  let encode { Diff.optional; mode; force_source = _; file1; file2 } input output : Sexp.t
    =
    let mode : Sexp.t =
      Atom
        (match mode with
...
let diff ?(optional = false) ?(mode = Diff.Mode.Text) ?(force_source = false) file1 file2 =
  Action.action { Diff.optional; mode; force_source; file1; file2 }
;;
