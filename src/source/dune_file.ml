...
module Dir_map = struct
  module Per_dir = struct
    ...
    type t =
      ...
      ; files : Files.t
      ; scope : Dune_lang.Scope_stanza.t list Filename.Map.t
      }
    ...
    let merge d1 d2 =
      ...
      ; scope = Filename.Map.union d1.scope d2.scope ~f:(fun _ v1 v2 -> Some (v1 @ v2))
      }
    ;;
  end
...
module Group = struct
  ...
  let combine t (ast : Ast.t) =
    ...
    | Scope s ->
      { t with
        scope = Filename.Map.add_multi t.scope (Dune_lang.Scope_stanza.directory s) s
      }
