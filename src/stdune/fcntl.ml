...
type lock_operation = F_SETLK | F_SETLKW | F_GETLK

type l_type = F_RDLCK | F_WRLCK | F_UNLCK
...
external fcntl_lk :
  ...
  -> int
  -> int * int
  = "fcntl_lk_bytecode" "fcntl_lk_native"

let of_type = function Read -> F_RDLCK | Write -> F_WRLCK

let lock ?(whence = Set) ?(start = 0) ?(len = 0) fd t =
  match fcntl_lk fd F_SETLKW (of_type t) whence start len with
  | -1, _ ->
      raise (Unix.Unix_error (Unix.EINTR, "fcntl", "F_SETLKW"))
  | 0, _ ->
      ()
  | r, _ ->
