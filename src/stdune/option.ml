...
module O = struct
  let ( >>| ) t f = match t with None -> None | Some a -> Some (f a)

  let ( >>= ) t f = match t with None -> None | Some a -> f a
...
let forall t ~f = match t with None -> true | Some t -> f t

let value t ~default = match t with Some x -> x | None -> default
