...
let eval (t : _ Status.Map.t) ~dirs =
  ...
  let to_set ~standard pred =
    String.Set.of_list (Predicate_lang.filter pred ~standard dirs) in
  let data_only = to_set ~standard:default.data_only t.data_only in
  let vendored =
    to_set ~standard:default.vendored t.vendored in
  ...
  | Some dir ->
    User_error.raise
      [ Pp.textf
          "Directory %s was marked as vendored and data_only, \
           it can't be marked as both."
          dir
      ]
