If the worker dies while the main process is sending `action/cancel-build`, the
build still fails promptly instead of waiting forever for that RPC response.
...
worker just after that. The main process still exits promptly.
