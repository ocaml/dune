After an action-runner stop-on-first-error cancellation in watch mode, the
reused action runner still recovers cleanly for the next build.
  ...
  > EOF
  $ build_quiet ok >/dev/null 2>&1; code=$?; [ "$code" = 0 ]
  $ stop_dune_quiet
