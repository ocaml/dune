Before dune 3.18, the substitution would only apply to absolute paths containing no spaces:
...
A longer test, which could be un-nested once dune itself requires 3.18 in its dune-project:
  ...
  >   $LEFT=$RIGHT:$SPACED=$SPACED:HOME=HOME:parent-env=parent-env:/workspace_root=/workspace_root:$TESTCASE_ROOT=$TESTCASE_ROOT:$TMPDIR=$TMPDIR:$RIGHT=$RIGHT
  > EOF
