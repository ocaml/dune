...
Set 'cache-check-probability' to 1.0, which should trigger the check
  ...
  $ rm -rf _build
  $ dune build --config-file config reproducible non-reproducible
  Warning: cache store error [06927586341dc59cfb5eff2c537d521f]: ((in_cache
...
Check that probability values less than zero and greater than one are rejected
  ...
  > EOF
  $ dune build --config-file config reproducible non-reproducible
  File "$TESTCASE_ROOT/config", line 4, characters 0-30:
  4 | (cache-check-probability -0.1)
      ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  ...
  $ dune build --config-file config reproducible non-reproducible
  File "$TESTCASE_ROOT/config", line 4, characters 0-30:
  4 | (cache-check-probability 3.14)
      ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  Error: The reproducibility check probability must be in the range [0, 1].
  [1]
...
Test that the environment variable and the command line flag work too
  ...
  $ rm -rf _build
  $ DUNE_CACHE_CHECK_PROBABILITY=0.0 DUNE_CACHE_USER_RULES=enabled dune build --cache=enabled reproducible non-reproducible

  $ rm -rf _build
  $ DUNE_CACHE_CHECK_PROBABILITY=1.0 DUNE_CACHE_USER_RULES=enabled dune build --cache=enabled reproducible non-reproducible
  Warning: cache store error [06927586341dc59cfb5eff2c537d521f]: ((in_cache
  ((non-reproducible 1c8fc4744d4cef1bd2b8f5e915b36be9))) (computed
  ((non-reproducible 6cfaa7a90747882bcf4ffe7252c1cf89)))) after executing
  (echo 'build non-reproducible';cp dep non-reproducible)
  ...
  $ rm -rf _build
  $ DUNE_CACHE_CHECK_PROBABILITY=1.0 DUNE_CACHE_USER_RULES=enabled dune build --cache-check-probability=0.0 --cache=enabled reproducible non-reproducible

  $ rm -rf _build
  $ DUNE_CACHE_USER_RULES=enabled dune build --cache=enabled --cache-check-probability=1.0 reproducible non-reproducible
  Warning: cache store error [06927586341dc59cfb5eff2c537d521f]: ((in_cache
  ((non-reproducible 1c8fc4744d4cef1bd2b8f5e915b36be9))) (computed
  ((non-reproducible 6cfaa7a90747882bcf4ffe7252c1cf89)))) after executing
  (echo 'build non-reproducible';cp dep non-reproducible)
