...
Will not create components with invalid names
  ...
  dune: NAME argument: invalid component name `invalid-component-name' Library
        names must be non-empty and composed only of the following characters:
        'A'..'Z', 'a'..'z', '_' or '0'..'9'.
  [1]
  $ test -f ./_test_lib
  [1]
...
The opam file should already match the generated result before the first
build:

  $ cp new_exec_proj/new_exec_proj.opam new_exec_proj.opam.before-build
...
And the opam file content is as expected
...
Building and installing the freshly initialized library project currently fails
...
Running the freshly initialized project's tests currently fails for the same
reason:
