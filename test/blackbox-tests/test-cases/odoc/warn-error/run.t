...
All calls to `odoc compile` and `odoc html` should have the `--warn-error` option.
  ...
  Running[0]: %OPAM_PREFIX%/bin/ocamlc.opt -config > /tmp/duneb1dd5f.output
  # Dune context:
  #  { name = "default"
  #  ; kind = "default"
  #  ; profile = Dyn
  ...
  Running[4]: (cd _build/default/lib/.foo.objs/byte && %OPAM_PREFIX%/bin/odoc compile --warn-error -I . -I ../../../_doc/_odoc/pkg/foo --pkg foo -o foo.odoc foo.cmti)
  Running[5]: (cd _build/default/_doc/_html && %OPAM_PREFIX%/bin/odoc html -I ../_odoc/pkg/foo -I ../../lib/.foo.objs/byte -o . ../_odoc/pkg/foo/page-index.odoc)
  Running[6]: (cd _build/default && %OPAM_PREFIX%/bin/odoc support-files -o _doc/_html)
