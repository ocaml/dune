For workspace package `foo`, `(deps (package foo))` computes the closure of its
libraries, then includes all installed files from their owning packages.
Libraries in those packages that are outside the closure do not become
additional roots. This library-based expansion does not follow dependencies
mentioned only in package metadata.
...
Requiring `namespace.selected` includes the whole namespace package. Its
generated META must retain the template's custom top-level variables.
...
The default implementation lives in the virtual library's package, so
materializing that package makes both libraries available. The nested Dune
consumer below will select the default implementation when linking.
...
These package dependencies supply the libraries needed by external OCaml
