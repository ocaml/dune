...
Create library B (the base dependency):
...
Create library A (depends on B) with two modules - one that uses base_lib,
one that doesn't:
...
Create a trivial executable that consumes middle_lib:
...
Now change base_lib's INTERFACE:
...
Standalone in middle_lib is recompiled even though it doesn't use base_lib:
  ...
  $ dune build ./main.exe --display short 2>&1 | grep Standalone
        ocamlc middle_lib/.middle_lib.objs/byte/middle_lib__Standalone.{cmi,cmti}
      ocamlopt middle_lib/.middle_lib.objs/native/middle_lib__Standalone.{cmx,o}
