Workspace-installed binaries are narrowed when using dune package management: a
package's stanzas resolve a sibling package's binary only when it is declared
as a dependency. This matches what the package sees when built in isolation
(opam-repo-ci), where an undeclared sibling isn't installed. See
