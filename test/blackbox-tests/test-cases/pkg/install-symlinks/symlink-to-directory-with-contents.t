...
code is exercised:
  ...
  >   (run sh -c "echo 'real content' > real_dir/real.txt")
  >   (run ln -s real.txt real_dir/link.txt)
  >   (run ln -s real_dir link_to_dir)
  >   (run mkdir -p %{lib}/%{pkg-self:name})
  >   (run cp link_to_dir/real.txt %{lib}/%{pkg-self:name}/)
  >   (run cp -P link_to_dir/link.txt %{lib}/%{pkg-self:name}/)
