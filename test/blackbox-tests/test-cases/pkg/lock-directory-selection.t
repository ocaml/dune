  ...
  >  (default
  >   (lock_dir (cond
  >    ((and (= %{architecture} arm64) (= %{system} macos)) dune.macos.arm64.lock)
  >    ((= %{system} macos) dune.macos.lock)
...
Build macos package on macos:
  $ dune clean
  $ DUNE_CONFIG__SYSTEM=macos DUNE_CONFIG__ARCHITECTURE=arm64 dune build _build/_private/default/.pkg/macos-only/target/
  arm64-only
  macos-only

Build macos package on macos:
  $ dune clean
  $ DUNE_CONFIG__SYSTEM=macos DUNE_CONFIG__ARCHITECTURE=amd64 dune build _build/_private/default/.pkg/macos-only/target/
  macos-only

Build linux package on macos (will fail):
  $ dune clean
  $ DUNE_CONFIG__SYSTEM=macos DUNE_CONFIG__ARCHITECTURE=amd64 dune build _build/_private/default/.pkg/linux-only/target/
  macos-only
...
Build macos package on linux (will fail):
  $ dune clean
  $ DUNE_CONFIG__SYSTEM=linux DUNE_CONFIG__ARCHITECTURE=amd64 dune build _build/_private/default/.pkg/macos-only/target/
  linux-only
...
Build linux package on linux:
  $ dune clean
  $ DUNE_CONFIG__SYSTEM=linux DUNE_CONFIG__ARCHITECTURE=amd64 dune build _build/_private/default/.pkg/linux-only/target/
  linux-only

Try setting the os to one which doesn't have a corresponding lockdir:
  $ dune clean
  $ DUNE_CONFIG__SYSTEM=windows dune build _build/_private/default/.pkg/linux-only/target/
  File "dune-workspace", line 28, characters 3-160:
  28 |    ((and (= %{architecture} arm64) (= %{system} macos)) dune.macos.arm64.lock)
  29 |    ((= %{system} macos) dune.macos.lock)
