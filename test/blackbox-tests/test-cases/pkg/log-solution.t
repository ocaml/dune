...
We should be seeing that it has created a lock dir solution depending on foo.0.0.1:
  ...
  $ jq '.[] | select(.cat == "log" and .args.message == "Dependency solution") | .args' trace.json
  {
...
We should be seeing that it has created a lock dir solution depending on foo.0.0.2:
  ...
  $ dune trace cat | jq '.[] | select(.cat == "log" and .args.message == "Dependency solution") | .args | del(.lock_dir)'
  {
    "message": "Dependency solution",
    "packages": [
      "foo.0.0.2"
