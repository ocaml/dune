...
Build as though we were on openbsd.
  ...
  $ cat _build/_private/default/.pkg/foo/target/share/kernel
  OpenBSD

Now building on linux won't work:
  $ DUNE_CONFIG__OS=linux DUNE_CONFIG__ARCH=x86_64 dune build
  ...
  Hint:
  (lock_dir
   (solve_for_platforms (with_defaults) (extra ((arch x86_64) (os linux)))))
  Hint: ...and then rerun 'dune pkg lock'
  [1]
  $ cat _build/_private/default/.pkg/foo/target/share/kernel
  OpenBSD

Add "(with_defaults)" to the "(solve_for_platforms ...)" field so the solution
includes the default platforms.
  $ cat > dune-workspace <<EOF
  > (lang dune 3.18)
  > (repository
  >  (name mock)
...
Now we can build on linux:
  $ DUNE_CONFIG__OS=linux DUNE_CONFIG__ARCH=x86_64 dune build
  $ cat _build/_private/default/.pkg/foo/target/share/kernel
  Other

And we can still build on openbsd:
  $ DUNE_CONFIG__OS=openbsd DUNE_CONFIG__ARCH=x86_64 dune build
  $ cat _build/_private/default/.pkg/foo/target/share/kernel
  OpenBSD
...
error case.
  ...
  $ DUNE_CONFIG__PORTABLE_LOCK_DIR=enabled dune pkg lock
  File "dune-workspace", line 7, characters 22-22:
  7 |  (solve_for_platforms))
                            
  Error: No platforms were specified for solving dependencies.
  Hint: Either set "(with_defaults)" or specify at least one platform in
  "(extra ...)".
  [1]
