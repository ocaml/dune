...
The log file will contain errors about the package being unavailable.
  $ sed _build/log -n -e "/Couldn't solve the package dependency formula./,\$p"
...
No errors when you try to build the platform on macos.
  $ DUNE_CONFIG__OS=macos DUNE_CONFIG__ARCH=x86_64 dune build

Building on linux fails because the lockdir doesn't contain a compatible solution.
  $ DUNE_CONFIG__OS=linux DUNE_CONFIG__ARCH=arm64 dune build
  ...
  Hint:
  (lock_dir
   (solve_for_platforms (with_defaults) (extra ((arch arm64) (os linux)))))
