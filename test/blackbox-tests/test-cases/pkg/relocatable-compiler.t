Test that relocatable-compiler presence bypasses toolchain cache.
...
CR-soon Alizter: Currently this is not the case, not because we are using
toolchain, but because the symlinks in the install step are not being resolved.
...
CR-soon Alizter: Toolchain cache should be empty but is not:
