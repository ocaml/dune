Test (wrapped (module_name Mylib) (transition "...")) — combines custom wrapper
name with deprecated unwrapped compat modules.

The library is named mylib_impl but exposes modules as Mylib.Foo, Mylib.Bar.
Deprecated unwrapped names (Foo, Bar) still work but produce warnings.

  $ dune build 2>&1 | grep -v ocamlc | grep -v ocamlopt
