...
let spawn_sleep command =
  ...
  Unix.create_process sh argv Unix.stdin Unix.stdout Unix.stderr |> Pid.of_int_exn
;;
...
let cleanup_iteration events process_watcher =
  ...
  let rec wait_for_shutdown () =
    ...
    | Job_complete_ready ->
      ...
      wait_for_shutdown ()
    | Fiber_fill_ivar _ -> wait_for_shutdown ()
  in
  wait_for_shutdown ();
  debug "got shutdown";
  Process_watcher.killall process_watcher Signal.Kill;
  while Process_watcher.running_count process_watcher > 0 do
    match Event.Queue.next events with
    | Shutdown _ -> ()
    | Job_complete_ready ->
      ignore (Process_watcher.wait_unix process_watcher : Fiber.fill list)
