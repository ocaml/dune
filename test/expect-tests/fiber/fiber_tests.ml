...
let%expect_test "Stream.parallel_iter doesn't leak" =
  ...
  let noise = test 1 ~iter_function:Fiber.Stream.In.parallel_iter in

  (* Check that the number of live words keeps on increasing because we are
     leaking memory: *)
  let delta = test 1_000 ~iter_function:naive_stream_parallel_iter in
  assert (delta > noise);

  (* Check that the number of live words is constant with this iter function: *)
  let delta = test 1_000 ~iter_function:Fiber.Stream.In.parallel_iter in
  assert (delta = noise);
  [%expect {||}]
