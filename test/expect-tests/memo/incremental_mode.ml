...
let%expect_test "non-incremental mode preserves early errors outside Memo nodes" =
  in_non_incremental_mode ~f:(fun () ->
    let time = ref 0 in
    let trace = ref [] in
    let log event = trace := (event, !time) :: !trace in
              ...
              Memo.run_with_error_handler
                (fun () ->
                   Memo.fork_and_join_unit
                     ...
                     (fun () -> delay_n_time_units time 10 |> Memo.of_reproducible_fiber))
                ~handle_error_no_raise:(fun _exn ->
                  log "early";
                  Fiber.return ())))
    in
    List.rev !trace |> List.iter ~f:(fun (event, time) -> printfn "%s@%d" event time));
  [%expect
    {|
    early@0
    late@10
