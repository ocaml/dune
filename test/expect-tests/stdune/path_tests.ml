...
let descendant p ~of_ =
  Dyn.Encoder.option Path.to_dyn (Path.descendant p ~of_)
  |> print_dyn

let is_descendant p ~of_ =
  Dyn.Encoder.bool (Path.is_descendant p ~of_)
  |> print_dyn

let explode s =
  let open Dyn.Encoder in
  let exploded = Path.explode (Path.of_string s) in
  option (list string) exploded
  |> print_dyn
...
let reach_for_running p ~from =
  Path.reach_for_running p ~from
  |> Dyn.Encoder.string
  |> print_dyn

let relative p s =
  Path.to_dyn (Path.relative p s)
  |> print_dyn

let insert_after_build_dir_exn p s =
  Path.insert_after_build_dir_exn p s
  |> Path.to_dyn
  |> print_dyn
...
let%expect_test _ =
  descendant (Path.relative build_dir "foo/bar")
...
let%expect_test _ = relative (of_string "/abs1") "";
  [%expect{|
External "/abs1"
|}]
...
let%expect_test _ =
  reach_for_running (Path.relative root "foo")
    ~from:(Path.relative root "foo");
  [%expect{|
"./."
|}]
