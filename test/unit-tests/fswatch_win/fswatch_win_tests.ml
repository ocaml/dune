...
let watch, collect_events =
  ...
  let rec collect_events acc = function
    ...
    | e :: events when Fswatch_win.Event.path e = end_of_test ->
      if not (List.is_empty events) then (
        Printf.printf
          "***** Leftover events after end of test marker event *****\n";
  ...
  let collect_events () =
    ...
    let events =
      ...
      | events ->
        Printf.printf
          "***** First event is not the beginning of test marker *****\n";
        collect_events [] events
    in
...
let _ =
  ...
  watch ".";
  create_file fn;
  check_events ~real_events:(collect_events ())
    [ { action = "modified"; path = fn } ]
...
let _ =
  ...
  let entries =
    List.map
      (gen_tree ~depth:1 ~files_per_dir:2 ~sub_dirs_per_dir:2)
      ~f:Stdlib.snd
  in
