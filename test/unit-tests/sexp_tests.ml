...
(* Test that all strings of length <= 3 such that [Galach.Atom.is_valid
   s] are recignized as atoms by the parser *)

let string_of_syntax (x : Galach.syntax) =
  match x with
  | Dune -> "dune"
  | Jbuild -> "jbuild"

let () =
  [ Galach.Dune, Galach.Lexer.token, (fun s -> Galach.Atom.is_valid s Dune)
  ; Jbuild, Galach.Lexer.jbuild_token, (fun s -> Galach.Atom.is_valid s Jbuild)
  ]
    ...
    for len = 0 to 3 do
      ...
      for i = 0 to 1 lsl (len * 8) - 1 do
        ...
        let parser_recognizes_as_atom =
          match Galach.parse_string ~lexer ~fname:"" ~mode:Single s with
          | exception _    -> false
          | Atom (_, A s') -> s = s'
          | _              -> false
        in
        let printed_as_atom =
          match Galach.atom_or_quoted_string s with
          | Atom _ -> true
          | _      -> false
        in
        let valid_dune_atom = validator (Galach.Atom.of_string s) in
        if valid_dune_atom <> parser_recognizes_as_atom then begin
          Printf.eprintf
            "Galach.Atom.is_valid error:\n\
             - syntax = %s\n\
             - s = %S\n\
             - Galach.Atom.is_valid s = %B\n\
        ...
        if printed_as_atom && not parser_recognizes_as_atom then begin
          Printf.eprintf
            "Galach.Atom.atom_or_quoted_string error:\n\
            - syntax = %s\n\
             - s = %S\n\
             - printed_as_atom = %B\n\
             - parser_recognizes_as_atom = %B\n"
