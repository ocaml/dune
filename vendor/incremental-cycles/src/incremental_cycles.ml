...
let rec visit_backward
  ...
  | [] -> VisitBackwardCompleted
  | vertex :: stack ->
    visit_backward_incoming g target mark vertex fuel stack (get_incoming g vertex)

and visit_backward_incoming g target mark vertex fuel stack = function
  ...
  | y :: incoming ->
    if fuel = 0 then
      ...
      visit_backward_incoming g target mark vertex (fuel - 1) stack incoming
    else if vertex_eq y target then begin
      ...
      visit_backward_incoming g target mark vertex (fuel - 1) (y :: stack) incoming
    end
...
let rec visit_forward
  ...
  match stack with
  | [] -> ForwardCompleted
  | x :: stack -> visit_forward_outgoing g new_level visited x stack (get_outgoing g x)

and visit_forward_outgoing g new_level visited x stack = function
  ...
  | y :: outgoing ->
    if is_marked g y visited then
      ...
      if y_level < new_level then begin
        ...
        clear_incoming g y;
        add_incoming g y x;
        visit_forward_outgoing g new_level visited x (y :: stack) outgoing
      end else if y_level = new_level then begin
        add_incoming g y x;
        visit_forward_outgoing g new_level visited x stack outgoing
